 ...
 */
const tenderDocumentForClient = async (document) => {
    const displayUrl = await TenderDocumentStorageService_1.tenderDocumentStorageService.displayUrl(String(document?.fileUrl || ''), {
...
const isForeignKeyConstraintError = (error) => {
    ...
    while (current && typeof current === 'object' && !seen.has(current)) {
        ...
        if (code === 1451 || code === '1451' || code === 'ER_ROW_IS_REFERENCED_2')
            return true;
        if (/foreign key constraint/i.test(String(current.message || '')))
            return true;
...
const normalizeIdList = (value) => Array.isArray(value)
    ...
    : [];
const isSourceSalesOrder = (value) => {
 ...
 */
const normalizeClosingImages = (raw) => {
    let list = [];
    if (Array.isArray(raw)) {
        ...
        try {
            ...
            if (Array.isArray(parsed))
                list = parsed.map((item) => String(item)).filter(Boolean);
        }
        catch {
...
const clampPositionLogText = (value) => {
 ...
 */
const sanitizeTenderCcEmails = (raw) => {
...
const looksLikeHtmlMessage = (value) => /<([a-z][a-z0-9]*)\b[^>]*>/i.test(value);
const MAIL_HTML_ALLOWED_TAGS = /^(b|strong|i|em|u|s|strike|ul|ol|li|br|p|div|span|font|h2|h3)$/i;
const sanitizeMailHtml = (html) => html.replace(/<\s*(\/?)\s*([a-z][a-z0-9]*)((?:[^>"']|"[^"]*"|'[^']*')*)>/gi, (_m, closing, tag, attrs) => {
    if (!MAIL_HTML_ALLOWED_TAGS.test(tag))
        return '';
    ...
    if (lower === 'font') {
        ...
        if (colorValue && /^#?[a-z0-9(),.%\s-]+$/i.test(colorValue))
            safeAttrs += ` color="${colorValue}"`;
        const size = String(attrs).match(/\bsize\s*=\s*(?:"([^"]*)"|'([^']*)'|([^\s>]+))/i);
        const sizeValue = size?.[1] ?? size?.[2] ?? size?.[3];
        if (sizeValue && /^[1-7]$/.test(sizeValue))
            safeAttrs += ` size="${sizeValue}"`;
    }
    ...
    if (styleValue) {
        const kept = styleValue
            .split(';')
            .map((rule) => rule.trim())
            .filter((rule) => /^(color|font-size)\s*:\s*[a-z0-9#(),.%\s-]+$/i.test(rule))
            .join('; ');
        if (kept)
...
class TenderController {
    ...
    async tenantRootId(tenantId) {
        return (0, tenantTree_1.findTenantRootIdCached)(tenantId);
    }
    ...
    async getAccessibleTender(tenderId, user, options) {
    ...
    async list(req, res) {
        try {
            ...
            if (req.query.tenderNumber)
                filter.tenderNumber = req.query.tenderNumber;
            if (req.query.customerName)
                filter.customerName = req.query.customerName;
            if (req.query.creatorName)
                filter.creatorName = req.query.creatorName;
            if (req.query.orderState === 'draft' || req.query.orderState === 'order')
                filter.orderState = req.query.orderState;
            if (req.query.mailSent === 'yes' || req.query.mailSent === 'no')
                filter.mailSent = req.query.mailSent;
            if (req.query.sortBy)
    ...
    async addPosition(req, res) {
        ...
        if (!Array.isArray(req.body?.positions)) {
            ...
            req.body = {
                positions: [{ clientId: `single-${(0, nanoid_1.nanoid)(8)}`, position }],
            };
            req.singlePositionResponse = true;
        ...
        try {
            ...
            if (safeRowType === 'PRODUCT' && sourceArticle && tender.customerId && (discount === undefined || discount === null)) {
                const customerDiscount = await prisma_client_1.default.customerProductDiscount.findFirst({
                    where: { customerId: tender.customerId, articleId: sourceArticle.id },
                    select: { discount: true },
                });
                if (customerDiscount)
                    resolvedDiscount = Number(customerDiscount.discount || 0);
            }
            const defaults = {
                SECTION: "Yeni bölüm",
                TITLE: "Başlık",
     ...
     */
    async addPositionsBatch(req, res) {
        ...
        try {
            ...
            const deleteIds = [...new Set(rawDeleteIds.map((value) => String(value || '').trim()).filter(Boolean))];
            ...
            const metaData = {};
            if (rawMeta.billingAddress !== undefined)
                metaData.billingAddress = rawMeta.billingAddress ? String(rawMeta.billingAddress) : null;
            if (rawMeta.installationAddress !== undefined)
                metaData.installationAddress = rawMeta.installationAddress ? String(rawMeta.installationAddress) : null;
            if (rawMeta.deliveryAddress !== undefined)
                metaData.deliveryAddress = rawMeta.deliveryAddress ? String(rawMeta.deliveryAddress) : null;
            if (rawMeta.commissionNumber !== undefined)
                metaData.commissionNumber = rawMeta.commissionNumber ? String(rawMeta.commissionNumber) : null;
            ...
            if (rawMeta.manualCustomerName !== undefined)
                metaData.manualCustomerName = rawMeta.manualCustomerName ? String(rawMeta.manualCustomerName).trim().slice(0, 190) : null;
            if (rawMeta.manualCustomerEmail !== undefined)
                metaData.manualCustomerEmail = rawMeta.manualCustomerEmail ? String(rawMeta.manualCustomerEmail).trim().slice(0, 190) : null;
            if (rawMeta.manualCustomerAddress !== undefined)
                metaData.manualCustomerAddress = rawMeta.manualCustomerAddress ? String(rawMeta.manualCustomerAddress) : null;
            if (rawMeta.customerReference !== undefined)
                metaData.customerReference = rawMeta.customerReference ? String(rawMeta.customerReference) : null;
            if (rawMeta.priceList !== undefined)
                metaData.priceList = rawMeta.priceList ? String(rawMeta.priceList) : null;
            // CC-Empfänger der Offerte (Speichern-Knopf läuft über DIESEN Endpunkt).
            if (rawMeta.ccEmails !== undefined)
                metaData.ccEmails = sanitizeTenderCcEmails(rawMeta.ccEmails);
            ...
            const canFastCreateEntries = entries.every(({ position, safeRowType, requestedParentPositionId }) => (safeRowType === 'PRODUCT' || safeRowType === 'CUSTOM')
            ...
            if (canUseFastSave) {
                ...
                if (fastCreated.length > 0) {
                    ...
                    const insertSelects = fastCreated.map(({ row }) => {
                        insertParameters.push(row.id, row.tenantId, row.tenderId, row.parentPositionId, row.rowType, row.sourceArticleId, row.displayOrder, row.npkCode, row.positionNumber, row.shortDescription, row.longDescription, row.quantity, row.unit, row.hierarchyLevel, row.imageUrl, row.unitPrice, row.discount, row.discounts, row.taxRate, tenantId, tenderId);
                        return `SELECT ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?
                                FROM \`Tender\` AS t
                                WHERE t.\`tenantId\` = ? AND t.\`id\` = ? AND t.\`status\` = 'Draft'`;
                    });
                    insertPromise = prisma_client_1.default.$executeRawUnsafe(`INSERT INTO \`Position\`
                ...
                const assignments = fastMutableFields.flatMap((field) => {
                    const matching = updates.filter(({ input }) => Object.prototype.hasOwnProperty.call(input, field));
                ...
                if (updateIds.length > 0) {
                    ...
                    updatePromise = prisma_client_1.default.$executeRawUnsafe(`UPDATE \`Position\` AS p
                         ...
                         WHERE p.\`tenantId\` = ?
                           AND p.\`tenderId\` = ?
                           AND t.\`status\` = 'Draft'
                           AND p.\`id\` IN (${updateIds.map(() => '?').join(', ')})`, ...parameters);
                }
                ...
                const pricingTouched = fastCreated.length > 0 || updates.some(({ input }) => input.quantity !== undefined
                ...
                if (pricingTouched) {
                    ...
                    void prisma_client_1.default.$executeRawUnsafe(`INSERT INTO \`CalculationItem\`
                         ...
                         ON DUPLICATE KEY UPDATE \`totalCalculatedPrice\` = VALUES(\`totalCalculatedPrice\`)`, tenantId, tenderId, ...pricingPositionIds).catch((calculationError) => console.error('[addPositionsBatch.fast] calculation sync failed:', calculationError));
                }
                ...
                if (fastActivityLogs.length > 0) {
                    void prisma_client_1.default.tenderActivityLog.createMany({
                    ...
                    }).catch((logError) => console.error('[addPositionsBatch.fast] activity log failed:', logError));
                }
                res.setHeader('Server-Timing', [
                    `auth;dur=${Number(req.authDurMs ?? 0)}`,
                    `rbac;dur=${Number(req.rbacDurMs ?? 0)}`,
                    ...stageDurations.map(([name, dur]) => `${name};dur=${dur}`),
                    `fast-save;dur=${Date.now() - requestStartedAt}`,
                ].join(', '));
                if (req.singlePositionResponse && fastCreated.length > 0) {
                ...
                if (req.singleUpdateResponse && updates.length > 0) {
                    return res.status(200).json({ id: updates[0].positionId, ...updates[0].input });
                }
                if (req.singleDeleteResponse) {
            ...
            const sourceArticleMap = new Map(sourceArticles.map((article) => [article.id, article]));
            ...
            const discountedArticleIds = tender.customerId
                ? [...new Set(entries
                        .filter((entry) => entry.safeRowType === 'PRODUCT'
            ...
            const customerDiscounts = discountedArticleIds.length > 0
                ? await prisma_client_1.default.customerProductDiscount.findMany({
                    where: { customerId: tender.customerId, articleId: { in: discountedArticleIds } },
                    select: { articleId: true, discount: true },
                })
                : [];
            const customerDiscountMap = new Map(customerDiscounts.map((item) => [item.articleId, Number(item.discount || 0)]));
            const defaults = {
                SECTION: "Yeni bölüm",
                TITLE: "Başlık",
                DESCRIPTION: "Yeni satır",
            ...
            const affectedPositionMap = new Map(affectedPositions.map((position) => [position.id, position]));
            const updateLabels = {
            ...
            const preparedUpdates = updates.map(({ positionId, input }) => {
                ...
                if (input.shortDescription !== undefined)
                    patch.shortDescription = String(input.shortDescription);
                if (input.longDescription !== undefined)
                    patch.longDescription = input.longDescription || null;
                if (targetCanPrice) {
                    if (input.quantity !== undefined)
                        patch.quantity = Number(input.quantity);
                    if (input.unit !== undefined)
                        patch.unit = input.unit || null;
                    if (input.unitPrice !== undefined)
                        patch.unitPrice = input.unitPrice === null ? null : Number(input.unitPrice);
                ...
                if (input.imageUrl !== undefined)
                    patch.imageUrl = targetCanHaveImage ? (input.imageUrl || null) : null;
                if (input.rowType !== undefined && !targetCanHaveImage)
                    patch.imageUrl = null;
                if (input.npkCode !== undefined)
                    patch.npkCode = input.npkCode || null;
                if (input.rowType !== undefined)
                ...
                if (input.displayOrder !== undefined)
                    patch.displayOrder = Number(input.displayOrder);
                if (Object.keys(patch).length === 0) {
                    throw new TenderValidationError("Güncellenecek satır alanı bulunamadı.");
                }
                const nextPosition = { ...before, ...patch };
                const pricingChanged = targetCanPrice && (input.quantity !== undefined
                    || input.unitPrice !== undefined
                    || input.discount !== undefined
            ...
            const needsDerivedOrdering = prepared.some((item) => item.requestedDisplayOrder === undefined || !item.requestedPositionNumber);
            ...
            const applyWrites = async (tx) => {
                ...
                if (preparedUpdates.length > 0) {
                    ...
                    const assignments = mutableFields.flatMap((field) => {
                        const matching = preparedUpdates.filter((entry) => Object.prototype.hasOwnProperty.call(entry.patch, field));
                        if (matching.length === 0)
                            return [];
                    ...
                    parameters.push(tenantId, tenderId, ...updateIds);
                    await tx.$executeRawUnsafe(`UPDATE \`Position\` SET ${assignments.join(', ')} WHERE \`tenantId\` = ? AND \`tenderId\` = ? AND \`id\` IN (${updateIds.map(() => '?').join(', ')})`, ...parameters);
                    ...
                    if (calculationUpdates.length > 0) {
                        ...
                        await tx.$executeRawUnsafe(`INSERT INTO \`CalculationItem\` (\`id\`, \`positionId\`, \`materialCost\`, \`laborCost\`, \`overheadCost\`, \`riskAmount\`, \`additionalCost\`, \`profitMargin\`, \`totalCalculatedPrice\`) VALUES ${valuesSql} ON DUPLICATE KEY UPDATE \`totalCalculatedPrice\` = VALUES(\`totalCalculatedPrice\`)`, ...calculationParameters);
                    }
                }
            ...
            const writeStatementCount = (Object.keys(metaData).length > 0
            ...
            if (needsDerivedOrdering || writeStatementCount > 1) {
                await prisma_client_1.default.$transaction(applyWrites, { isolationLevel: 'ReadCommitted', maxWait: 5000, timeout: 15000 });
            }
            else {
                await applyWrites(prisma_client_1.default);
            }
            ...
            if (activityLogs.length > 0) {
                void prisma_client_1.default.tenderActivityLog.createMany({ data: activityLogs })
                    .catch((logError) => console.error('[addPositionsBatch] activity log write failed:', logError));
            }
            res.setHeader('Server-Timing', `auth;dur=${Number(req.authDurMs ?? 0)}, rbac;dur=${Number(req.rbacDurMs ?? 0)}, validation;dur=${validationFinishedAt - requestStartedAt}, db-write;dur=${writeFinishedAt - validationFinishedAt}, total;dur=${writeFinishedAt - requestStartedAt}`);
                ...
                ? {
                    ...
                    ...(requestedCustomerId !== undefined
                        ? {
                            ...
                            customerAddress: (0, customerAddress_1.formatCustomerAddress)(metaCustomer),
    ...
    async updateMeta(req, res) {
        try {
            ...
            const ccOnlyPatch = ccEmails !== undefined
                && Object.keys(req.body).filter((key) => req.body[key] !== undefined).length === 1;
    ...
    async updatePosition(req, res) {
        if (!Array.isArray(req.body?.updates)) {
            ...
            req.body = {
                positions: [],
                updates: [{ positionId: req.params.positionId, patch }],
                deleteIds: [],
            };
            req.singleUpdateResponse = true;
            return this.addPositionsBatch(req, res);
        }
        try {
            const tenderId = req.params.id;
            ...
            const before = await prisma_client_1.default.position.findFirst({
                where: {
                    id: positionId,
                    tenderId,
                    tenantId,
            ...
            if (!before) {
                ...
                const existing = await prisma_client_1.default.position.findFirst({
            ...
            const changedLogs = Object.keys(patch)
                ...
                newValue: nextPosition[field] == null ? null : String(nextPosition[field]),
                description: `${labels[field] ?? field} değiştirildi: ${before[field] ?? 'boş'} -> ${nextPosition[field] ?? 'boş'}`
            }));
            if (patch.imageUrl !== undefined) {
                changedLogs.push({
                    tenantId,
            ...
            if (pricingChanged) {
                ...
                if (qty > 0 && price != null) {
                    ...
                    writes.push(prisma_client_1.default.calculationItem.upsert({
                        where: { positionId },
    ...
    async cancel(req, res) {
        try {
            ...
            await prisma_client_1.default.$transaction(async (tx) => {
                await (0, documentLifecycle_1.cancelTenderWithin)(tx, {
                    tenderId,
                    tenantId: tender.tenantId,
                    employeeId: req.user.id,
                    reason,
                ...
                await (0, documentGovernance_1.recordDocumentEvent)(tx, {
                    ...
                    actorId: req.user.id,
                    reason,
                    snapshot: { previousStatus: tender.status ?? null, version: tender.version ?? null },
    ...
    async delete(req, res) {
        try {
            ...
            if (!lifecycle.canDelete && req.body?.override) {
                const decision = await (0, documentGovernance_1.decideOverride)({
                ...
                if (!decision.ok)
                    return res.status(decision.status).json((0, documentGovernance_1.overrideErrorBody)(decision));
            ...
            if (override) {
                const parked = await prisma_client_1.default.appointment.findMany({
                    where: { tenantId: tender.tenantId, detachedFromTenderId: tenderId, startTime: { gte: new Date() }, NOT: { status: 'CANCELLED' } },
                    select: { id: true },
                });
                const cancellations = await Promise.all(parked.map((row) => (0, calendarMailService_1.buildAppointmentCancellation)(row.id).catch(() => null)));
                if (parked.length) {
            ...
            await (0, documentGovernance_1.recordDocumentEvent)(prisma_client_1.default, {
                ...
                snapshot: { version: tender.version ?? null, status: tender.status ?? null, cancelledAppointments },
                links: { projectId: tender.revertedProject?.id ?? null },
                ipAddress: (0, documentGovernance_1.requestIp)(req),
            }).catch((error) => console.error('[TenderController.delete] Verlauf nicht geschrieben:', error?.message || error));
    ...
    async listMailDrafts(req, res) {
        try {
            const tenantId = req.user.tenantId;
            const drafts = await prisma_client_1.default.tenderMailDraft.findMany({
                where: { tenantId },
    ...
    async createMailDraft(req, res) {
        try {
            const tenantId = req.user.tenantId;
            const { subject, message } = req.body;
            const draft = await prisma_client_1.default.tenderMailDraft.create({
                data: {
                    id: (0, nanoid_1.nanoid)(10),
                    tenantId,
    ...
    async updateMailDraft(req, res) {
        try {
            const tenantId = req.user.tenantId;
            const draftId = req.params.draftId;
            const existing = await prisma_client_1.default.tenderMailDraft.findFirst({
            ...
            if (subject !== undefined)
                data.subject = String(subject ?? '').slice(0, 191);
            if (message !== undefined)
                data.message = message ? String(message) : null;
            if (Object.keys(data).length === 0) {
                return res.status(400).json({ error: "Güncellenecek alan bulunamadı." });
            }
            const draft = await prisma_client_1.default.tenderMailDraft.update({
                where: { id: draftId },
                data,
            });
    ...
    async deleteMailDraft(req, res) {
        try {
            ...
            if (!existing)
                return res.status(404).json({ error: "Taslak bulunamadı." });
            await prisma_client_1.default.tenderMailDraft.delete({ where: { id: draftId } });
    ...
    async listTextTemplates(req, res) {
        try {
            const tenantId = req.user.tenantId;
            const templates = await prisma_client_1.default.tenderTextTemplate.findMany({
                where: { tenantId },
    ...
    async createTextTemplate(req, res) {
        try {
            const tenantId = req.user.tenantId;
            const { title, content, isDefault } = req.body;
            const template = await prisma_client_1.default.$transaction(async (tx) => {
    ...
    async updateTextTemplate(req, res) {
        try {
            const tenantId = req.user.tenantId;
            const templateId = req.params.templateId;
            const existing = await prisma_client_1.default.tenderTextTemplate.findFirst({
            ...
            const data = {};
            if (title !== undefined)
                data.title = String(title ?? '').slice(0, 191);
            if (content !== undefined)
                data.content = content ? String(content) : null;
    ...
    async deleteTextTemplate(req, res) {
        try {
            const tenantId = req.user.tenantId;
            const templateId = req.params.templateId;
            const existing = await prisma_client_1.default.tenderTextTemplate.findFirst({
            ...
            if (!existing)
                return res.status(404).json({ error: "Şablon bulunamadı." });
            await prisma_client_1.default.tenderTextTemplate.delete({ where: { id: templateId } });
    ...
    async sendOfferMail(req, res) {
        try {
            ...
            const contacts = tender.customerId
                ? await prisma_client_1.default.customerContact.findMany({
            ...
            if (tender.customerId) {
                ...
                if (result.mailMessageId && activity?.id) {
                    await prisma_client_1.default.mailMessage.update({
                        where: { id: result.mailMessageId },
                        data: { activityId: String(activity.id) },
                    }).catch(() => undefined);
                }
            }
            res.status(200).json({
     ...
     */
    async listMailRecipients(req, res) {
        try {
            const tenderId = req.params.id;
            const tender = await this.getAccessibleTender(tenderId, req.user, { omitPdfContent: true });
            if (!tender)
            ...
            res.status(200).json({
                customer: tender.customerEmail
                    ...
                    : null,
                contacts: contacts
                    .filter((contact) => contact.email && isValidEmail(String(contact.email).trim()))
                    .map((contact) => ({
                    id: contact.id,
    ...
    async getDetails(req, res) {
        try {
            ...
            const [tender, positions, activities] = await Promise.all([
                this.getAccessibleTender(tenderId, req.user, {
    ...
    // needs them.
    async getPdfContent(req, res) {
        try {
            const tenderId = req.params.id;
            if (!tenderId)
                return res.status(400).json({ error: "İhale ID zorunludur." });
            const requestTenantId = req.user.tenantId;
            const owner = await prisma_client_1.default.tender.findUnique({
                where: { id: tenderId },
                select: { tenantId: true },
            });
            ...
            const content = await prisma_client_1.default.tender.findFirst({
    ...
    async getPdfImages(req, res) {
        try {
            ...
            if (ids.length === 0 && positionIds.length === 0)
            ...
            const [articleThumbs, positionThumbs] = await Promise.all([
                ...
                (0, PdfImageThumbnailService_1.getPositionThumbnails)(tenantId, tenderId, positionRows),
            ]);
    ...
    async deletePosition(req, res) {
        if (!Array.isArray(req.body?.deleteIds)) {
            req.body = {
                positions: [],
                updates: [],
                deleteIds: [req.params.positionId],
            };
            req.singleDeleteResponse = true;
            return this.addPositionsBatch(req, res);
        }
        try {
            const tenderId = req.params.id;
    ...
    async getChatter(req, res) {
        try {
            ...
            const [logs, documents] = await Promise.all([
                ...
                prisma_client_1.default.document.findMany({
                    where: { tenantId: tender.tenantId, relatedEntityId: tender.id, entityType: "TENDER" },
    ...
    async getDocumentContent(req, res) {
        try {
            ...
            let document = null;
            if (normalizedRef) {
                const documents = await prisma_client_1.default.$queryRaw(client_1.Prisma.sql `
                    SELECT document.*
            ...
            if (!document) {
                ...
                if (!tender)
                    return res.status(404).json({ error: "Teklif bulunamadı." });
                document = await prisma_client_1.default.document.findFirst({
                    where: {
                        id: documentId,
                        tenantId: tender.tenantId,
            ...
            if (TenderDocumentStorageService_1.tenderDocumentStorageService.isManagedReference(document.fileUrl)) {
                const fileUrl = await TenderDocumentStorageService_1.tenderDocumentStorageService.displayUrl(document.fileUrl, {
                    contentType: document.fileType || undefined,
                });
                if (!fileUrl) {
    ...
    async addDocument(req, res) {
        ...
        try {
            ...
            if (uploadedFile || !/^https?:\/\//i.test(fileUrl)) {
                ...
                const stored = await TenderDocumentStorageService_1.tenderDocumentStorageService.store(tenantId, body, fileType);
                storedFileReference = TenderDocumentStorageService_1.tenderDocumentStorageService.publicReadUrl(stored);
                if (!storedFileReference) {
                    await TenderDocumentStorageService_1.tenderDocumentStorageService.remove(stored).catch(() => undefined);
            ...
            const normalizedRef = this.normalizeTenderRef(tenderRef);
            if (normalizedRef) {
                const documentId = (0, nanoid_1.nanoid)(8);
                const inserted = await prisma_client_1.default.$executeRaw(client_1.Prisma.sql `
            ...
            if (!tender) {
                if (storedFileReference) {
                    await TenderDocumentStorageService_1.tenderDocumentStorageService.remove(storedFileReference);
            ...
            if (storedFileReference) {
                await TenderDocumentStorageService_1.tenderDocumentStorageService.remove(storedFileReference).catch((cleanupError) => {
     ...
     */
    async correctTexts(req, res) {
        try {
            const tenderId = req.params.id;
            const user = req.user;
            const tender = await this.getAccessibleTender(tenderId, user, { omitPdfContent: true });
            if (!tender)
                return res.status(404).json({ error: "Teklif bulunamadı." });
            if (tender.status === 'Cancelled' || tender.cancelledAt) {
            ...
            const TEXT_FIELDS = { shortDescription: 500, longDescription: 20000, unit: 40 };
            const META_FIELDS = { billingAddress: 2000, installationAddress: 2000, deliveryAddress: 2000, commissionNumber: 191, customerReference: 191 };
            const logValue = (value) => (value == null ? null : String(value).slice(0, 2000));
            ...
            for (const row of Array.isArray(req.body?.positions) ? req.body.positions : []) {
                ...
                if (Object.keys(data).length)
                    positionPatches.push({ id: row.id, data });
            }
            const metaPatch = {};
            ...
            const current = positionPatches.length
                ? await prisma_client_1.default.position.findMany({
            ...
            const metaBefore = Object.keys(metaPatch).length
                ? await prisma_client_1.default.tender.findUnique({
                    where: { id: tenderId },
            ...
            await prisma_client_1.default.$transaction(async (tx) => {
                for (const patch of positionPatches) {
                    ...
                    for (const [field, value] of Object.entries(patch.data)) {
                        ...
                        data[field] = value;
                        logs.push({
                            id: (0, nanoid_1.nanoid)(12), tenantId: tender.tenantId, tenderId, positionId: patch.id,
                ...
                for (const [field, value] of Object.entries(metaPatch)) {
                    if ((metaBefore?.[field] ?? null) === value)
                        continue;
                    metaData[field] = value;
                    logs.push({
                        id: (0, nanoid_1.nanoid)(12), tenantId: tender.tenantId, tenderId,
                        employeeId: user.id, actionType: 'TEXT_CORRECTION', fieldName: field,
    ...
    async listSnapshots(req, res) {
        try {
            ...
            if (!tender)
                return res.status(404).json({ error: "Teklif bulunamadı." });
            const rows = await prisma_client_1.default.tenderSnapshot.findMany({
                where: { tenderId, tenantId: tender.tenantId },
                orderBy: { createdAt: 'desc' },
            });
            const employeeIds = [...new Set(rows.map((row) => row.createdById).filter(Boolean))];
