...
exports.ADDON_READ_PERMISSIONS = [
 ...
 */
const parsePaymentStagesInput = (raw) => {
    if (raw === undefined)
        return undefined;
    if (raw === null || raw === '')
        return { value: null };
    ...
    if (!stages)
        return { error: 'Ödeme planı geçersiz.' };
    const stageError = (0, paymentSchedule_1.validatePaymentStages)(stages);
    if (stageError)
...
class AddonOrderController {
    ...
    async list(req, res) {
        try {
            ...
            const conditions = [
                client_1.Prisma.sql `so.tenantId = ${tenantId}`,
                client_1.Prisma.sql `so.parentSalesOrderId IS NOT NULL`,
            ];
            if (req.query.parentSalesOrderId) {
                conditions.push(client_1.Prisma.sql `so.parentSalesOrderId = ${String(req.query.parentSalesOrderId)}`);
            }
            if (req.query.projectId) {
                conditions.push(client_1.Prisma.sql `so.projectId = ${String(req.query.projectId)}`);
            }
            ...
            const rowsPromise = prisma_client_1.default.$queryRaw(client_1.Prisma.sql `
                SELECT
            ...
            // laufen (wie `myOrders`).
            const invoicesPromise = prisma_client_1.default.$queryRaw(client_1.Prisma.sql `
            ...
            res.status(200).json(rows.map((row) => ({
                ...
                createdBy: row.creatorFirstName || row.creatorLastName
                    ...
                    : null,
                billingSummary: (0, SalesOrderController_1.listBillingFigures)(summaries.get(row.id)),
            })));
        }
        catch (error) {
     ...
     */
    async loadAddonLines(addon) {
        const projectId = addon.projectId;
        const parentId = addon.parentSalesOrderId;
        const siblings = await prisma_client_1.default.salesOrder.findMany({
        ...
        return {
            materials: materials.map((row) => ({
                ...
                documentLine: (0, addonDocumentLines_1.readAddonLineMetadata)(row.documentLine),
                addedAt: row.addedAt,
            ...
            overtime: reports
                .filter((row) => Number(row.overtimeCost || 0) > 0)
                .map((row) => ({
                id: row.id,
    ...
    async document(req, res) {
        try {
            ...
            const addon = await prisma_client_1.default.salesOrder.findFirst({
            ...
            if (!addon)
                return res.status(404).json({ error: 'Zusatzauftrag nicht gefunden.' });
            const lines = await this.loadAddonLines(addon);
            const invoiceCount = await prisma_client_1.default.invoice.count({ where: { salesOrderId: addon.id, tenantId, ...invoiceDrafts_1.issuedInvoiceWhere } });
    ...
    async create(req, res) {
        try {
            ...
            if (!rawParentId)
                return res.status(400).json({ error: 'Hauptauftrag fehlt.' });
            const selected = await prisma_client_1.default.salesOrder.findFirst({ where: { id: rawParentId, tenantId } });
            if (!selected)
                return res.status(404).json({ error: 'Hauptauftrag nicht gefunden.' });
            // Wer einen Nachtrag anklickt, meint dessen Hauptauftrag.
            const parent = selected.parentSalesOrderId
                ? await prisma_client_1.default.salesOrder.findFirst({ where: { id: selected.parentSalesOrderId, tenantId } })
                : selected;
            if (!parent)
                return res.status(404).json({ error: 'Hauptauftrag nicht gefunden.' });
            ...
            const { products, texts } = (0, addonDocumentLines_1.normalizeAddonLines)(req.body?.lines);
    ...
    async replaceLines(req, res) {
        try {
            ...
            if (!addon)
                return res.status(404).json({ error: 'Zusatzauftrag nicht gefunden.' });
            if (!addon.projectId)
                return res.status(400).json({ error: 'Dieser Zusatzauftrag trägt kein Projekt.' });
            ...
            const invoiceCount = await prisma_client_1.default.invoice.count({ where: { salesOrderId: addon.id, tenantId, ...invoiceDrafts_1.issuedInvoiceWhere } });
            if (invoiceCount > 0)
                return res.status(400).json({ error: 'Ein fakturierter Zusatzauftrag kann nicht mehr geändert werden.' });
            const hasLines = req.body?.lines !== undefined;
            const { products, texts } = hasLines ? (0, addonDocumentLines_1.normalizeAddonLines)(req.body.lines) : { products: [], texts: [] };
            const orderDate = parseOrderDate(req.body?.orderDate);
            if (orderDate === undefined && req.body?.orderDate !== undefined) {
                return res.status(400).json({ error: 'Datum ungültig.' });
            }
            ...
            const articleIds = [...new Set(products.map((line) => line.articleId))];
            const articles = articleIds.length
                ? await prisma_client_1.default.article.findMany({ where: { id: { in: articleIds }, tenantId }, select: { id: true, salePrice: true, name: true } })
                : [];
            const articleById = new Map(articles.map((article) => [article.id, article]));
            ...
            const updated = await prisma_client_1.default.$transaction(async (tx) => {
                if (hasLines) {
                    ...
                    const existingMaterials = await tx.projectExtraMaterial.findMany({
                    ...
                    const existingById = new Map(existingMaterials.map((row) => [row.id, row]));
                    for (const product of products) {
                        const line = (0, addonDocumentLines_1.priceAddonProduct)(product, articleById.get(product.articleId));
...
exports.AddonOrderController = AddonOrderController;
//# sourceMappingURL=AddonOrderController.js.map