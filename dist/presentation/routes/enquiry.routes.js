...
exports.publicEnquiryRouter = void 0;
...
const WRITE = (0, RbacMiddleware_1.requirePermission)("crm.activities.create");
...
const publicSubmitLimiter = (0, RateLimitMiddleware_1.rateLimit)({
    windowMs: 15 * 60 * 1000,
    max: 8,
    message: "Zu viele Anfragen. Bitte versuchen Sie es spaeter erneut.",
});
const publicReadLimiter = (0, RateLimitMiddleware_1.rateLimit)({ windowMs: 15 * 60 * 1000, max: 120 });
const parsePage = (req) => {
...
const newToken = () => crypto_1.default.randomBytes(24).toString("base64url");
const ensureForm = async (tenantId) => {
    const existing = await prisma_client_1.default.enquiryForm.findUnique({ where: { tenantId } });
    if (existing)
        return existing;
    try {
        return await prisma_client_1.default.enquiryForm.create({
            data: { id: (0, nanoid_1.nanoid)(12), tenantId, token: newToken(), active: true },
        });
    }
    catch {
        // Wettlauf zweier Reiter: einer gewinnt, der andere liest ihn.
        return prisma_client_1.default.enquiryForm.findUniqueOrThrow({ where: { tenantId } });
    }
};
const formDto = (form) => ({
    token: form.token,
 ...
 */
router.get("/", AuthMiddleware_1.requireAuth, READ, async (req, res) => {
    try {
        const user = req.user;
        ...
        const where = [client_1.Prisma.sql `e.tenantId = ${user.tenantId}`];
        if (statusRaw === "OPEN")
            where.push(client_1.Prisma.sql `e.status IN (${client_1.Prisma.join(OPEN_STATUSES)})`);
        else if (STATUSES.has(statusRaw))
            where.push(client_1.Prisma.sql `e.status = ${statusRaw}`);
        if (SOURCES.has(source))
            where.push(client_1.Prisma.sql `e.source = ${source}`);
        if (assignedEmployeeId)
            where.push(client_1.Prisma.sql `e.assignedEmployeeId = ${assignedEmployeeId}`);
        if (from)
            where.push(client_1.Prisma.sql `e.createdAt >= ${from}`);
        if (to)
            where.push(client_1.Prisma.sql `e.createdAt <= ${to}`);
        if (search) {
            const like = `%${search}%`;
            where.push(client_1.Prisma.sql `(e.companyName LIKE ${like} OR e.contactName LIKE ${like} OR e.email LIKE ${like} OR e.subject LIKE ${like})`);
        }
 ...
 */
router.get("/stats", AuthMiddleware_1.requireAuth, READ, async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
        const rows = await prisma_client_1.default.enquiry.groupBy({
            by: ["status"],
            where: { tenantId },
            _count: { _all: true },
        });
...
router.get("/form", AuthMiddleware_1.requireAuth, READ, async (req, res) => {
    try {
...
router.patch("/form", AuthMiddleware_1.requireAuth, WRITE, async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
        ...
        if (req.body?.title !== undefined)
            data.title = text(req.body.title, 200);
        if (req.body?.intro !== undefined)
            data.intro = text(req.body.intro, 4000);
        if (req.body?.thanks !== undefined)
            data.thanks = text(req.body.thanks, 2000);
        ...
        const updated = await prisma_client_1.default.enquiryForm.update({ where: { tenantId }, data });
...
router.post("/form/rotate", AuthMiddleware_1.requireAuth, WRITE, async (req, res) => {
    try {
        const tenantId = req.user.tenantId;
        await ensureForm(tenantId);
        const updated = await prisma_client_1.default.enquiryForm.update({ where: { tenantId }, data: { token: newToken() } });
        res.json(formDto(updated));
    }
    catch (error) {
        res.status(400).json({ error: error?.message });
...
const loadOne = async (id, tenantId) => {
...
router.get("/:id", AuthMiddleware_1.requireAuth, READ, async (req, res) => {
    try {
...
router.post("/", AuthMiddleware_1.requireAuth, WRITE, async (req, res) => {
    try {
        const user = req.user;
        ...
        const created = await prisma_client_1.default.enquiry.create({
            data: {
 ...
 * aendert.
 */
router.patch("/:id", AuthMiddleware_1.requireAuth, WRITE, async (req, res) => {
    try {
        const user = req.user;
        const id = String(req.params.id);
        const existing = await prisma_client_1.default.enquiry.findFirst({
            where: { id, tenantId: user.tenantId }, select: { id: true },
        });
        if (!existing)
            return res.status(404).json({ error: "Anfrage nicht gefunden." });
        const data = {};
        const setText = (field, key, max) => {
            if (req.body?.[key] !== undefined)
                data[field] = text(req.body[key], max);
        };
        ...
        if (req.body?.priority !== undefined) {
            ...
            if (!PRIORITIES.has(priority))
                return res.status(400).json({ error: "Prioritaet unbekannt." });
        ...
        if (req.body?.status !== undefined) {
            ...
            if (!STATUSES.has(status))
                return res.status(400).json({ error: "Stand unbekannt." });
            data.status = status;
        ...
        await prisma_client_1.default.enquiry.update({ where: { id }, data });
 ...
 */
router.post("/:id/convert", AuthMiddleware_1.requireAuth, (0, RbacMiddleware_1.requirePermission)("crm.customers.create"), async (req, res) => {
    try {
        const user = req.user;
        const id = String(req.params.id);
        const enquiry = await prisma_client_1.default.enquiry.findFirst({ where: { id, tenantId: user.tenantId } });
        if (!enquiry)
            return res.status(404).json({ error: "Anfrage nicht gefunden." });
        ...
        if (!companyName)
            return res.status(400).json({ error: "Ohne Firma oder Name laesst sich kein Kunde anlegen." });
        ...
        const customer = await prisma_client_1.default.customer.create({
            data: {
        ...
        await prisma_client_1.default.enquiry.update({
            where: { id },
...
router.delete("/:id", AuthMiddleware_1.requireAuth, WRITE, async (req, res) => {
    try {
        const user = req.user;
        const id = String(req.params.id);
        const existing = await prisma_client_1.default.enquiry.findFirst({
        ...
        if (!existing)
            return res.status(404).json({ error: "Anfrage nicht gefunden." });
        await prisma_client_1.default.enquiry.delete({ where: { id } });
        res.status(204).send();
    }
    catch (error) {
        res.status(400).json({ error: error?.message });
...
/* ─────────────────────────────────────────────────────────────────────────────
   ...
   ───────────────────────────────────────────────────────────────────────── */
exports.publicEnquiryRouter = (0, express_1.Router)();
/** Beschreibung des Formulars — Titel, Einleitung, Feldregeln, Firmenname. */
exports.publicEnquiryRouter.get("/:token", publicReadLimiter, async (req, res) => {
    try {
        const form = await prisma_client_1.default.enquiryForm.findUnique({
        ...
        if (!form || !form.active)
            return res.status(404).json({ error: "Dieses Formular ist nicht (mehr) verfuegbar." });
        res.json({
...
exports.publicEnquiryRouter.post("/:token", publicSubmitLimiter, async (req, res) => {
    try {
        const form = await prisma_client_1.default.enquiryForm.findUnique({
            where: { token: String(req.params.token) },
        ...
        if (text(req.body?.website, 200))
            return res.status(201).json({ ok: true, thanks: form.thanks || null });
        ...
        if (!subject)
            return res.status(400).json({ error: "Bitte einen Betreff angeben." });
        if (!message)
            return res.status(400).json({ error: "Bitte eine Nachricht schreiben." });
        ...
        const known = await prisma_client_1.default.customer.findFirst({
            where: { tenantId: form.tenantId, mainEmail: email },
            select: { id: true },
        });
        await prisma_client_1.default.enquiry.create({
            data: {
                id: (0, nanoid_1.nanoid)(12),
