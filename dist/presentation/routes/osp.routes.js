...
webhookSpecs) => {
    ...
    const specs = (0, ospDatasheet_1.mergeSpecs)((0, ospDatasheet_1.mergeSpecs)(previous?.datasheetSpecs, result.specs), webhookSpecs);
...
router.get('/units/:unitId/datasheet', AuthMiddleware_1.requireAuth, (0, RbacMiddleware_1.requirePermission)('tenders.view'), async (req, res) => {
    try {
        ...
        if (!unit.datasheetFile)
            return res.status(404).json({ error: 'Zu dieser Einheit liegt kein Datenblatt.' });
        const body = await LocalFileStorage_1.ospDatasheetStorage.read(unit.datasheetFile);
