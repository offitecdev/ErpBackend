...
exports.armedWebhookKeys = exports.invalidateOspWebhookKeys = void 0;
 ...
 */
const storeUnitDatasheet = async (setting, unitId, url, 
...
webhookSpecs) => {
    if (!setting)
        return { ok: false, error: 'OSP ist noch nicht konfiguriert.' };
    const previous = await prisma_client_1.default.ospUnit.findUnique({
        where: { id: unitId },
        select: {
    ...
    const remote = declaredMarkdown ? await (0, ospDatasheet_1.fetchOspMarkdown)(setting, declaredMarkdown) : null;
    ...
    const base = fresh ?? previous?.datasheetSpecs;
    const specs = (0, ospDatasheet_1.mergeSpecs)((0, ospDatasheet_1.mergeSpecs)(base, remote?.ok ? remote.specs : null), webhookSpecs);
    const markdown = (remote?.ok ? remote.markdown : null) || result.markdown || null;
    await prisma_client_1.default.ospUnit.update({
        where: { id: unitId },
        data: result.ok
            ? {
 ...
 */
const readUnitDatasheet = async (setting, unit) => {
    if (unit.datasheetFile) {
        try {
            return { ok: true, body: await LocalFileStorage_1.ospDatasheetStorage.read(unit.datasheetFile) };
        }
        catch {
    ...
    const fetched = await storeUnitDatasheet(setting, unit.id, unit.pdfUrl, (0, ospDatasheet_1.specsFromOfferEntry)(unit.rawPayload));
    if (!fetched.ok) {
        return {
            ok: false,
            status: 502,
    ...
    const fresh = await prisma_client_1.default.ospUnit.findUnique({
    ...
    return { ok: true, body: await LocalFileStorage_1.ospDatasheetStorage.read(fresh.datasheetFile) };
};
/**
...
let webhookKeyCache = null;
const invalidateOspWebhookKeys = () => {
    webhookKeyCache = null;
};
exports.invalidateOspWebhookKeys = invalidateOspWebhookKeys;
const armedWebhookKeys = async () => {
    if (webhookKeyCache && webhookKeyCache.expiresAt > Date.now())
        return webhookKeyCache.keys;
    const rows = await prisma_client_1.default.ospSetting.findMany({
        where: { NOT: { webhookKey: null } },
        select: { id: true, webhookKey: true },
    });
    const keys = rows
        .map((row) => ({ id: String(row.id), webhookKey: asTrimmed(row.webhookKey) || '' }))
        .filter((row) => row.webhookKey);
    webhookKeyCache = { expiresAt: Date.now() + WEBHOOK_KEY_CACHE_TTL_MS, keys };
    return keys;
};
exports.armedWebhookKeys = armedWebhookKeys;
const authenticateWebhook = async (req, res) => {
    const key = asTrimmed(req.header('x-osp-integration-key'));
    const armed = await (0, exports.armedWebhookKeys)();
    if (!armed.length) {
    ...
    const setting = await prisma_client_1.default.ospSetting.findUnique({ where: { id: match.id } });
    if (!setting || !asTrimmed(setting.webhookKey)) {
        // Zeile ist seit dem Zwischenspeichern weg oder entwaffnet.
        (0, exports.invalidateOspWebhookKeys)();
        res.status(401).json({ message: 'Missing or wrong X-OSP-Integration-Key.' });
        return null;
    }
    return setting;
...
const webhookRateLimiter = (0, RateLimitMiddleware_1.rateLimit)({
    windowMs: 15 * 60 * 1000,
    max: 120,
    ...
    for (const row of rows) {
        ...
        const units = details
            ? details
                ...
                pdfUrl: (0, ospDatasheet_1.pickDatasheetUrl)(detail),
                markdownUrl: (0, ospDatasheet_1.pickMarkdownUrl)(detail),
                changes: asChangeList(detail.changes),
                specs: (0, ospDatasheet_1.specsFromOfferEntry)(detail),
                raw: detail,
            }))
                .filter((unit) => unit.ospDocumentId)
            : (documentId
                ...
                ? [{
                        ...
                        pdfUrl: (0, ospDatasheet_1.pickDatasheetUrl)(entry),
                        markdownUrl: (0, ospDatasheet_1.pickMarkdownUrl)(entry),
                        changes: asChangeList(entry.changes),
                        specs: (0, ospDatasheet_1.specsFromOfferEntry)(entry),
                        raw: entry,
                    }]
                : []);
        out.push({
 ...
 */
const findRequestRow = async (tenantId, projectNumber) => {
    const exact = await prisma_client_1.default.ospDocument.findUnique({
        where: { tenantId_reference: { tenantId, reference: projectNumber } },
    });
    if (exact)
        return exact;
    const legacy = await prisma_client_1.default.ospDocument.findMany({
        where: { tenantId, projectNumber },
        orderBy: { createdAt: 'asc' },
    });
    if (!legacy.length)
        return null;
    const rank = (row) => (OspClient_1.OSP_STATUS_RANK[row.status] ?? -1);
    const primary = [...legacy].sort((a, b) => {
        if (Boolean(a.tenderId) !== Boolean(b.tenderId))
            return a.tenderId ? -1 : 1;
        if (rank(a) !== rank(b))
            return rank(b) - rank(a);
        return new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime();
    })[0];
    return prisma_client_1.default.ospDocument.update({
        where: { id: primary.id },
 ...
 */
const ingestOfferProjects = async (setting, projects, mode) => {
    const out = {
    ...
    for (const project of projects) {
        ...
        if (existing) {
            ...
            row = await prisma_client_1.default.ospDocument.update({ where: { id: existing.id }, data });
            out.updated += 1;
            if (revived)
            ...
            row = await prisma_client_1.default.ospDocument.create({
                data: {
                    id: (0, nanoid_1.nanoid)(12), tenantId: setting.tenantId, reference: project.reference,
                    status: 'LISTED', ...data,
                    ...(mode === 'REVISION' && project.changes ? { changes: project.changes } : {}),
                },
            });
            out.created += 1;
 ...
 */
const ingestUnits = async (setting, row, project, mode) => {
    const datasheetJobs = [];
    let touched = 0;
    for (const unit of project.units) {
        const existing = await prisma_client_1.default.ospUnit.findUnique({
        ...
        const specs = (0, ospDatasheet_1.mergeSpecs)(existing?.datasheetSpecs, unit.specs);
        const data = {
            datasheetSpecs: (Object.keys(specs).length ? specs : null),
            changes: (unit.changes ?? null),
            receivedAt: new Date(),
            rawPayload: unit.raw,
        };
        if (unit.pdfUrl)
            data.pdfUrl = unit.pdfUrl;
        // Nennt die OSP eine Markdown-Fassung, ist sie die bessere Quelle —
        // heute tut sie es nicht, der Weg steht trotzdem.
        if (unit.markdownUrl)
            data.markdownUrl = unit.markdownUrl;
        let saved;
        if (existing) {
            saved = await prisma_client_1.default.ospUnit.update({ where: { id: existing.id }, data });
        }
        else {
            ...
            const known = await prisma_client_1.default.ospFeedEntry.findUnique({
            ...
            saved = await prisma_client_1.default.ospUnit.create({
                data: {
    ...
    if (mode === 'NEW' && project.hasUnitList && project.units.length) {
        ...
        const stale = await prisma_client_1.default.ospUnit.findMany({
            where: { requestId: row.id, ospDocumentId: { notIn: keep } },
            select: { id: true, datasheetFile: true },
        });
        if (stale.length) {
            await prisma_client_1.default.ospUnit.deleteMany({ where: { id: { in: stale.map((unit) => unit.id) } } });
            for (const unit of stale) {
                if (unit.datasheetFile)
                    await LocalFileStorage_1.ospDatasheetStorage.remove(unit.datasheetFile).catch(() => undefined);
            }
        }
    }
    ...
    try {
        ...
        for (const raw of rows) {
            ...
            for (const detail of details) {
                ...
                const pdfUrl = (0, ospDatasheet_1.pickDatasheetUrl)(detail);
                const feed = {
                    ...project,
                ...
                await prisma_client_1.default.ospFeedEntry.upsert({
                    where: { tenantId_ospDocumentId: { tenantId: setting.tenantId, ospDocumentId } },
                    create: {
                        id: (0, nanoid_1.nanoid)(12), tenantId: setting.tenantId, ospDocumentId,
                ...
                if (pdfUrl && pdfUrl !== held.pdfUrl) {
                    ...
                    jobs.push({ unitId: held.id, url: pdfUrl, specs: (0, ospDatasheet_1.specsFromOfferEntry)(detail) });
                }
                if (Object.keys(unitData).length) {
                    await prisma_client_1.default.ospUnit.update({ where: { id: held.id }, data: unitData }).catch(() => undefined);
                }
...
router.get('/documents', AuthMiddleware_1.requireAuth, (0, RbacMiddleware_1.requirePermission)('tenders.view'), async (req, res) => {
    try {
        ...
        if (items.length) {
            const units = await prisma_client_1.default.ospUnit.findMany({
                where: { requestId: { in: items.map((doc) => doc.id) } },
                orderBy: { createdAt: 'asc' },
            });
            ...
            for (const unit of units) {
                ...
                if (list)
                    ...
                    byRequest.set(unit.requestId, [unit]);
            }
            for (const doc of items)
                doc.units = byRequest.get(doc.id) || [];
        }
...
router.delete('/documents/:id', AuthMiddleware_1.requireAuth, (0, RbacMiddleware_1.requirePermission)('tenders.manage'), async (req, res) => {
    try {
        ...
        const files = [
            doc.datasheetFile,
            ...(doc.units || []).map((unit) => unit.datasheetFile),
        ].filter(Boolean);
        for (const file of files)
            await LocalFileStorage_1.ospDatasheetStorage.remove(file).catch(() => undefined);
...
const findUnit = async (rootId, unitId) => (prisma_client_1.default.ospUnit.findFirst({
 ...
 * ist.
 */
router.get('/units/:unitId/datasheet', AuthMiddleware_1.requireAuth, (0, RbacMiddleware_1.requirePermission)('tenders.view'), async (req, res) => {
    try {
        const feed = await loadFeedContext(req.user.tenantId);
        if (!feed?.visible)
            return res.status(403).json({ error: 'OSP ist für diese Firma nicht freigeschaltet.' });
        ...
        if (!unit)
            return res.status(404).json({ error: 'OSP-Einheit nicht gefunden.' });
        const read = await readUnitDatasheet(feed.setting, unit);
        if (!read.ok)
            return res.status(read.status).json({ error: read.error });
 ...
 */
router.post('/units/:unitId/datasheet', AuthMiddleware_1.requireAuth, (0, RbacMiddleware_1.requirePermission)('tenders.manage'), async (req, res) => {
    try {
        const feed = await loadFeedContext(req.user.tenantId);
        if (!feed?.visible)
            return res.status(403).json({ error: 'OSP ist für diese Firma nicht freigeschaltet.' });
        ...
        if (!url)
            return res.status(400).json({ error: 'Die OSP hat zu dieser Einheit keine Datenblatt-Adresse geliefert.' });
        if (url !== unit.pdfUrl) {
            await prisma_client_1.default.ospUnit.update({ where: { id: unit.id }, data: { pdfUrl: url } });
        }
        ...
        await storeUnitDatasheet(feed.setting, unit.id, url, (0, ospDatasheet_1.specsFromOfferEntry)(unit.rawPayload));
        res.json(await prisma_client_1.default.ospUnit.findUnique({ where: { id: unit.id } }));
    }
    catch (error) {
...
const firstUnitOf = async (rootId, requestId) => (prisma_client_1.default.ospUnit.findFirst({
...
router.post('/documents/:id/datasheet', AuthMiddleware_1.requireAuth, (0, RbacMiddleware_1.requirePermission)('tenders.manage'), async (req, res) => {
    try {
        ...
        await storeUnitDatasheet(feed.setting, unit.id, url, (0, ospDatasheet_1.specsFromOfferEntry)(unit.rawPayload));
        res.json(await prisma_client_1.default.ospDocument.findUnique({
 ...
 */
router.get('/units/:unitId/markdown', AuthMiddleware_1.requireAuth, (0, RbacMiddleware_1.requirePermission)('tenders.view'), async (req, res) => {
    try {
        const feed = await loadFeedContext(req.user.tenantId);
        if (!feed?.visible)
            return res.status(403).json({ error: 'OSP ist für diese Firma nicht freigeschaltet.' });
        const unit = await prisma_client_1.default.ospUnit.findFirst({
...
router.post('/documents/:id/datasheets', AuthMiddleware_1.requireAuth, (0, RbacMiddleware_1.requirePermission)('tenders.manage'), async (req, res) => {
    try {
        const feed = await loadFeedContext(req.user.tenantId);
        if (!feed?.visible)
            return res.status(403).json({ error: 'OSP ist für diese Firma nicht freigeschaltet.' });
        ...
        for (const unit of units) {
            ...
            if (unit.datasheetFile && !force) {
                ...
                const readable = await LocalFileStorage_1.ospDatasheetStorage.read(unit.datasheetFile).then(() => true).catch(() => false);
                if (readable) {
                    results.push({ unitId: unit.id, ospDocumentId: unit.ospDocumentId, ok: true, skipped: true });
                    continue;
                }
            }
            const outcome = await storeUnitDatasheet(feed.setting, unit.id, unit.pdfUrl, (0, ospDatasheet_1.specsFromOfferEntry)(unit.rawPayload));
            results.push({
                unitId: unit.id,
                ospDocumentId: unit.ospDocumentId,
                ok: outcome.ok,
        ...
        res.json({
            ...
            results,
            document: await prisma_client_1.default.ospDocument.findUnique({
                where: { id: doc.id },
                include: { units: { orderBy: { createdAt: 'asc' } } },
            }),
        });
...
router.get('/feed', AuthMiddleware_1.requireAuth, (0, RbacMiddleware_1.requirePermission)('tenders.view'), async (req, res) => {
    try {
        const feed = await loadFeedContext(req.user.tenantId);
        if (!feed)
            return res.status(403).json({ error: 'Kein aktiver Firmenbaum.' });
        ...
        const [items, total] = await Promise.all([
            prisma_client_1.default.ospFeedEntry.findMany({
                where,
                orderBy: [{ filedAt: 'desc' }, { createdAt: 'desc' }],
                skip: (page - 1) * pageSize,
                take: pageSize,
            }),
            prisma_client_1.default.ospFeedEntry.count({ where }),
        ]);
        ...
        const held = items.length
            ? await prisma_client_1.default.ospUnit.findMany({
                where: {
                    tenantId: feed.rootId,
                    ospDocumentId: { in: items.map((row) => row.ospDocumentId) },
                },
                select: {
                    ospDocumentId: true,
                    requestId: true,
            ...
            : [];
        const byDocument = new Map(held.map((unit) => [unit.ospDocumentId, unit]));
    ...
    try {
        ...
        for (let start = 0; start < projectNumbers.length; start += 15) {
            ...
            const results = await Promise.all(chunk.map(async (projectNumber) => ({
                projectNumber,
                result: await (0, OspClient_1.fetchOspOfferStatus)(feed.setting, projectNumber),
            })));
            for (const { projectNumber, result } of results) {
                ...
                if (row?.status)
                    byProject.set(projectNumber, row);
            }
        }
        let updated = 0;
