...
const readReportFilters = (query) => {
    const start = (0, personnel_1.parseDateOnly)(query.startDate);
    const end = (0, personnel_1.parseDateOnly)(query.endDate);
    if (!start || !end || end < start)
        return null;
    return {
        start: (0, personnel_1.startOfDay)(start),
        end: (0, personnel_1.endOfDay)(end),
...
router.get('/shift-plan', AuthMiddleware_1.requireAuth, async (req, res) => {
    try {
...
router.put('/shift-plan', AuthMiddleware_1.requireAuth, (0, RbacMiddleware_1.requirePermission)('attendance.update'), async (req, res) => {
    try {
        const plan = (0, personnel_1.parseShiftPlan)({
            workdays: req.body?.workdays,
            startTime: (0, personnel_1.normalizeTime)(req.body?.startTime, personnel_1.DEFAULT_SHIFT_PLAN.startTime),
            endTime: (0, personnel_1.normalizeTime)(req.body?.endTime, personnel_1.DEFAULT_SHIFT_PLAN.endTime),
        ...
        const tenantId = await shiftPlanTenantId(req);
        await prisma_client_1.default.staffShiftPlan.upsert({
            where: { tenantId },
            create: {
 ...
 */
router.get('/staff', AuthMiddleware_1.requireAuth, (0, RbacMiddleware_1.requirePermission)('employees.view'), async (req, res) => {
    try {
        const tenantIds = await treeOf(req);
 ...
 */
router.post('/staff/bulk', AuthMiddleware_1.requireAuth, (0, RbacMiddleware_1.requirePermission)('employees.create'), async (req, res) => {
    try {
        const rawRows = Array.isArray(req.body?.rows) ? req.body.rows : [];
        if (rawRows.length === 0)
            return fail(res, 400, 'Mindestens eine Zeile wird benötigt.');
        if (rawRows.length > 100)
            return fail(res, 400, 'Höchstens 100 Zeilen je Sendung.');
        const entries = [];
        ...
        for (let index = 0; index < rawRows.length; index += 1) {
            ...
            if (!firstName && !lastName && !email && !password)
                continue;
            if (!firstName)
                return res.status(400).json({ error: 'Vorname fehlt.', index });
            if (!lastName)
                return res.status(400).json({ error: 'Nachname fehlt.', index });
            if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email))
                return res.status(400).json({ error: 'E-Mail-Adresse ist ungültig.', index });
            if (seenEmails.has(email))
                return res.status(400).json({ error: 'E-Mail-Adresse kommt in der Sendung doppelt vor.', index });
            try {
            ...
            entries.push({
                ...
                email,
                password,
                staffRole: (0, personnel_1.isStaffRole)(row.staffRole) ? String(row.staffRole) : 'STAFF',
                workLocation: (0, personnel_1.isWorkLocation)(row.workLocation) ? String(row.workLocation) : 'OFFICE',
            });
        }
        if (entries.length === 0)
            return fail(res, 400, 'Mindestens eine Zeile wird benötigt.');
        ...
        const taken = await prisma_client_1.default.employee.findMany({
        ...
        const created = await prisma_client_1.default.$transaction(entries.map((entry, position) => prisma_client_1.default.employee.create({
            data: {
                id: (0, nanoid_1.nanoid)(),
                tenantId,
 ...
 */
router.post('/staff/:id/qr', AuthMiddleware_1.requireAuth, (0, RbacMiddleware_1.requirePermission)('employees.update'), async (req, res) => {
    try {
        const tenantIds = await treeOf(req);
        ...
        await prisma_client_1.default.employee.update({ where: { id: existing.id }, data: { qrToken } });
        AuditLogService_1.auditLog.log({
...
const approverRowsSql = (tenantIds) => client_1.Prisma.sql `
    ...
    WHERE e.tenantId IN (${client_1.Prisma.join(tenantIds)})
      ...
      AND EXISTS (
            ...
            WHERE er.employeeId = e.id
              AND p.permissionName IN (${client_1.Prisma.join(APPROVER_PERMISSIONS)})
      )
 ...
 */
const isPurserEmployee = async (employeeId) => {
...
const purserCount = async (tenantIds) => {
    if (tenantIds.length === 0)
        return 0;
    const rows = await prisma_client_1.default.$queryRaw(client_1.Prisma.sql `
...
router.get('/approvers', AuthMiddleware_1.requireAuth, async (req, res) => {
    try {
        const tenantIds = await treeOf(req);
        if (tenantIds.length === 0)
            return res.status(200).json([]);
        const rows = await prisma_client_1.default.$queryRaw(approverRowsSql(tenantIds));
...
const isApprover = async (employeeId, tenantIds) => {
    if (!employeeId || tenantIds.length === 0)
        return false;
    const rows = await prisma_client_1.default.$queryRaw(client_1.Prisma.sql `
        SELECT e.id
        FROM Employee e
 ...
 */
router.post('/clock/scan', AuthMiddleware_1.requireAuth, async (req, res) => {
    try {
        const token = String(req.body?.token ?? '').trim();
        if (!token)
            return fail(res, 400, 'Kein QR-Code erkannt.');
        const tenantIds = await treeOf(req);
        const person = await staffByQrToken(token, tenantIds);
        if (!person)
            return fail(res, 404, 'Dieser QR-Code gehört zu keiner aktiven Person.');
        const now = new Date();
        const today = (0, personnel_1.startOfDay)(now);
        ...
        // „Pause" heisst.
        const todaysEntries = await prisma_client_1.default.staffTimeEntry.findMany({
        ...
        const open = todaysEntries.find((entry) => entry.endedAt === null)
            ?? await prisma_client_1.default.staffTimeEntry.findFirst({
                where: { employeeId: person.id, endedAt: null },
                orderBy: { startedAt: 'desc' },
                select: { id: true, startedAt: true, endedAt: true, durationSeconds: true },
            });
        const action = (0, personnel_1.scanTagFor)(plan, {
        ...
        if (open) {
            ...
            await prisma_client_1.default.staffTimeEntry.update({
                where: { id: open.id },
                data: { endedAt: now, durationSeconds },
            });
        }
        else {
            await prisma_client_1.default.staffTimeEntry.create({
                data: {
                    id: (0, nanoid_1.nanoid)(),
 ...
 */
router.get('/clock/activity', AuthMiddleware_1.requireAuth, async (req, res) => {
    try {
        const tenantIds = await treeOf(req);
        if (tenantIds.length === 0)
            return res.status(200).json({ date: null, events: [] });
        const day = (0, personnel_1.startOfDay)((0, personnel_1.parseDateOnly)(req.query.date) ?? new Date());
        const plan = await loadShiftPlan(req);
        const rows = await prisma_client_1.default.$queryRaw(client_1.Prisma.sql `
        ...
        for (const [employeeId, spans] of byEmployee) {
            const head = spans[0];
            const derived = (0, personnel_1.deriveDayActivity)(spans.map((row) => ({
                startedAt: new Date(row.startedAt),
                endedAt: row.endedAt ? new Date(row.endedAt) : null,
                durationSeconds: row.durationSeconds == null ? null : Number(row.durationSeconds),
            })), plan);
            const summary = (0, personnel_1.summariseDay)(spans.map((row) => ({
                startedAt: new Date(row.startedAt),
                endedAt: row.endedAt ? new Date(row.endedAt) : null,
                durationSeconds: row.durationSeconds == null ? null : Number(row.durationSeconds),
            })));
 ...
 */
router.get('/clock/week', AuthMiddleware_1.requireAuth, async (req, res) => {
    try {
        const tenantIds = await treeOf(req);
        if (tenantIds.length === 0)
            return res.status(200).json({ weekStart: null, days: [] });
        const anchor = (0, personnel_1.parseDateOnly)(req.query.weekStart) ?? new Date();
        const monday = (0, personnel_1.startOfIsoWeek)(anchor);
        const friday = (0, personnel_1.endOfDay)((0, personnel_1.addDays)(monday, 4));
        const rows = await prisma_client_1.default.$queryRaw(client_1.Prisma.sql `
        ...
        const days = Array.from({ length: 5 }, (_, offset) => {
            const date = (0, personnel_1.addDays)(monday, offset);
            const key = (0, personnel_1.toDateKey)(date);
            const entries = rows
                .filter((row) => (0, personnel_1.toDateKey)(new Date(row.workDate)) === key)
                .map((row) => ({
                id: String(row.id),
...
// Berichte
// ─────────────────────────────────────────────────────────────────────────────
router.get('/reports/detailed', AuthMiddleware_1.requireAuth, (0, RbacMiddleware_1.requirePermission)('attendance.read'), async (req, res) => {
    try {
        ...
        const plan = await loadShiftPlan(req);
        const tenantIds = await treeOf(req);
        const report = await (0, personnelReports_1.buildDetailedReport)(tenantIds, filters, plan);
...
router.get('/reports/accounting/:employeeId', AuthMiddleware_1.requireAuth, (0, RbacMiddleware_1.requirePermission)('attendance.read'), async (req, res) => {
    try {
        ...
        const plan = await loadShiftPlan(req);
        const tenantIds = await treeOf(req);
        const detail = await (0, personnelReports_1.buildAccountingDetail)(tenantIds, String(req.params.employeeId), filters, plan, holidays);
        if (!detail.person)
            return fail(res, 404, 'Person nicht gefunden.');
...
router.patch('/time-entries/:id', AuthMiddleware_1.requireAuth, (0, RbacMiddleware_1.requirePermission)('attendance.update'), async (req, res) => {
    try {
        const tenantIds = await treeOf(req);
        ...
        if (Number.isNaN(startedAt.getTime()))
            return fail(res, 400, 'Beginn ist ungültig.');
        if (endedAt && Number.isNaN(endedAt.getTime()))
            return fail(res, 400, 'Ende ist ungültig.');
        if (endedAt && endedAt < startedAt)
            return fail(res, 400, 'Das Ende liegt vor dem Beginn.');
        const updated = await prisma_client_1.default.staffTimeEntry.update({
            where: { id: entry.id },
            data: {
                startedAt,
                endedAt,
                durationSeconds: endedAt ? Math.max(0, Math.round((endedAt.getTime() - startedAt.getTime()) / 1000)) : null,
                workDate: (0, personnel_1.startOfDay)(startedAt),
                source: 'MANUAL',
...
router.delete('/time-entries/:id', AuthMiddleware_1.requireAuth, (0, RbacMiddleware_1.requirePermission)('attendance.update'), async (req, res) => {
    try {
        ...
        if (!entry)
            return fail(res, 404, 'Zeile nicht gefunden.');
        await prisma_client_1.default.staffTimeEntry.delete({ where: { id: entry.id } });
        AuditLogService_1.auditLog.log({
 ...
 */
const requestTypeCondition = (requestType) => {
    switch (requestType) {
 ...
 */
router.get('/leaves', AuthMiddleware_1.requireAuth, async (req, res) => {
    try {
        ...
        if (typeCondition)
            extra.push(typeCondition);
        const status = String(req.query.status || '').trim();
        if (personnel_1.LEAVE_STATUSES.includes(status))
            extra.push({ status });
        ...
        // beide Monatslisten.
        const from = (0, personnel_1.parseDateOnly)(req.query.from);
        if (from)
            extra.push({ endDate: { gte: (0, personnel_1.startOfDay)(from) } });
        const to = (0, personnel_1.parseDateOnly)(req.query.to);
        if (to)
            extra.push({ startDate: { lte: (0, personnel_1.endOfDay)(to) } });
        const search = String(req.query.search || '').trim();
        if (search) {
            extra.push({
                employee: {
        ...
        if (scope === 'mine') {
            ...
            const branches = [{ approverId: req.user.id }];
            ...
            if (!(await isPurserEmployee(req.user.id)))
                return fail(res, 403, 'Diese Ansicht führt der Purser.');
            where.kind = 'LEAVE';
            ...
            if (!permissions.includes('leaves.read'))
                return fail(res, 403, 'Für die Gesamtübersicht fehlt die Berechtigung.');
        }
        else {
            where.employeeId = req.user.id;
        }
        if (extra.length)
            where.AND = extra;
        const rows = await prisma_client_1.default.staffLeaveRequest.findMany({
            where,
            select: LEAVE_SELECT,
 ...
 */
router.post('/leaves', AuthMiddleware_1.requireAuth, async (req, res) => {
    try {
        const requestType = String(req.body?.requestType ?? '').trim();
        const derived = (0, personnel_1.isRequestType)(requestType)
            ? (0, personnel_1.requestTypeToLeave)(requestType, req.body?.leaveType)
            : null;
        const kind = derived
            ? derived.kind
        ...
        const leaveType = derived
            ? derived.leaveType
            : (kind === 'REMOTE' ? personnel_1.REMOTE_LEAVE_TYPE : String(req.body?.leaveType ?? ''));
        if (kind === 'LEAVE' && !(0, personnel_1.isLeaveType)(leaveType)) {
            return fail(res, 400, `Urlaubsart muss eine von ${personnel_1.LEAVE_TYPES.join(', ')} sein.`);
        }
        ...
        let leaveTypeLabel = null;
        if (kind === 'LEAVE' && (0, personnel_1.requiresLeaveTypeLabel)(leaveType)) {
            leaveTypeLabel = String(req.body?.leaveTypeLabel ?? '').trim().slice(0, personnel_1.LEAVE_TYPE_LABEL_MAX);
            if (!leaveTypeLabel)
                return fail(res, 400, 'Bitte die Urlaubsart als Text angeben.');
        }
        ...
        if (approverId === req.user.id)
            return fail(res, 400, 'Ein Antrag kann nicht bei der eigenen Person eingereicht werden.');
        const plan = await loadShiftPlan(req);
        ...
        (0, leaveRequestMailService_1.queueLeaveRequestNotice)(created.id, 'MANAGER');
        res.status(201).json(created);
    }
    catch (error) {
        fail(res, 400, error.message);
 ...
 */
router.patch('/leaves/:id/decision', AuthMiddleware_1.requireAuth, async (req, res) => {
    try {
        const approve = req.body?.decision === 'APPROVE';
        const reject = req.body?.decision === 'REJECT';
        if (!approve && !reject)
            return fail(res, 400, 'Entscheidung muss APPROVE oder REJECT sein.');
        ...
        if (request.status === 'PENDING_MANAGER') {
            ...
            /* Wohin der Antrag nach dem Ja geht. Urlaub gehört zum PURSER —
               ...
               Stufe lief. */
            let nextStatus = reject ? 'REJECTED' : (0, personnel_1.nextStatusAfterManagerApproval)(request.kind);
            ...
            const updated = await prisma_client_1.default.staffLeaveRequest.update({
                where: { id: request.id },
                data: {
                    status: nextStatus,
            ...
            (0, leaveRequestMailService_1.queueLeaveRequestNotice)(updated.id, nextStatus === 'PENDING_ACCOUNTING' ? 'ACCOUNTING' : 'DECIDED');
        ...
        if (request.status === 'PENDING_ACCOUNTING') {
            ...
            (0, leaveRequestMailService_1.queueLeaveRequestNotice)(updated.id, 'DECIDED');
            return res.status(200).json(updated);
        }
    ...
    try {
        ...
        const [approver, accounting, mine] = await Promise.all([
            ...
            amPurser
                ...
                : Promise.resolve(0),
            prisma_client_1.default.staffLeaveRequest.count({
 ...
 */
router.patch('/staff/:id/role', AuthMiddleware_1.requireAuth, (0, RbacMiddleware_1.requireAnyPermission)(['employees.update', 'roles.manage']), async (req, res) => {
    try {
        const tenantIds = await treeOf(req);
        const existing = await prisma_client_1.default.employee.findFirst({
            where: { id: String(req.params.id), tenantId: { in: tenantIds }, deletedAt: null },
        ...
        if (req.body?.staffRole !== undefined) {
            if (!(0, personnel_1.isStaffRole)(req.body.staffRole))
                return fail(res, 400, 'Unbekannte Personalrolle.');
        ...
        if (req.body?.workLocation !== undefined) {
            if (!(0, personnel_1.isWorkLocation)(req.body.workLocation))
                return fail(res, 400, 'Unbekannter Arbeitsort.');
            data.workLocation = String(req.body.workLocation);
        }
        if (Object.keys(data).length === 0)
            return fail(res, 400, 'Nichts zu ändern.');
        const updated = await prisma_client_1.default.employee.update({
 ...
 */
router.get('/staff/:id/overview', AuthMiddleware_1.requireAuth, async (req, res) => {
    try {
        const user = req.user;
        ...
        const now = new Date();
        const since = (0, personnel_1.addDays)((0, personnel_1.startOfDay)(now), -90);
        const until = (0, personnel_1.addDays)((0, personnel_1.startOfDay)(now), 365);
        const [tasks, meetings, appointments, leaves, approvals, passwordRequest] = await Promise.all([
            ...
            prisma_client_1.default.$queryRaw(client_1.Prisma.sql `
                ...
                LIMIT 60
            `),
            prisma_client_1.default.passwordChangeRequest.findFirst({
                where: { employeeId: id, status: 'PENDING' },
                select: { id: true, createdAt: true, note: true },
            }),
        ]);
        const assignedRole = (person.employeeRoles[0]?.role ?? null);
        const nameOf = (first, last) => `${String(first ?? '')} ${String(last ?? '')}`.trim() || null;
        res.status(200).json({
            person: {
                id: person.id,
...
const photoProblem = (value, limit) => {
    if (value.length > limit)
        return 'Das Bild ist zu gross. Bitte ein kleineres Foto wählen.';
    if (!PHOTO_PREFIX.test(value))
        return 'Nur JPEG-, PNG- oder WebP-Bilder sind möglich.';
    return null;
};
router.put('/staff/:id/photo', AuthMiddleware_1.requireAuth, async (req, res) => {
    try {
        const user = req.user;
        const id = String(req.params.id || '');
        if (id !== user.id) {
...
router.get('/photos', AuthMiddleware_1.requireAuth, async (req, res) => {
    try {
        ...
        for (const row of rows) {
            if (row.photo)
                photos[String(row.id)] = String(row.photo);
        }
