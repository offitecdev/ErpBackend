    ...
    try {
        ...
        if (scope === 'mine') {
            ...
            if (me?.staffRole !== 'ACCOUNTANT')
                return fail(res, 403, 'Diese Ansicht führt die Buchhaltung.');
            where.kind = 'LEAVE';
...
router.post('/leaves', AuthMiddleware_1.requireAuth, async (req, res) => {
    try {
        const kind = String(req.body?.kind) === 'REMOTE' ? 'REMOTE' : 'LEAVE';
        const leaveType = kind === 'REMOTE' ? personnel_1.REMOTE_LEAVE_TYPE : String(req.body?.leaveType ?? '');
    ...
    try {
        ...
        if (request.status === 'PENDING_MANAGER') {
            ...
            if (nextStatus === 'PENDING_ACCOUNTING') {
                const accountants = await prisma_client_1.default.employee.count({
