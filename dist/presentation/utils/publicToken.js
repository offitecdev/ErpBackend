...
exports.isBookingLinkExpired = exports.isSignatureLinkExpired = exports.readPublicToken = exports.PUBLIC_TOKEN_HEADER = void 0;
/**
 ...
 */
exports.PUBLIC_TOKEN_HEADER = 'x-public-token';
const readPublicToken = (req) => {
    const fromHeader = req.header(exports.PUBLIC_TOKEN_HEADER);
    if (fromHeader && fromHeader.trim())
        return fromHeader.trim();
    // Rückfallweg: der alte Weg mit dem Schlüssel im Pfad.
    return String(req.params.token || '').trim();
};
exports.readPublicToken = readPublicToken;
/**
 ...
 */
const readTtlDays = (envVar, fallback) => {
 ...
 */
const isBookingLinkExpired = (plannedDate) => {
    if (!plannedDate)
        return false;
    const days = readTtlDays('OFFITEC_BOOKING_LINK_TTL_DAYS', 7);
    return Date.now() > plannedDate.getTime() + days * DAY_MS;
};
exports.isBookingLinkExpired = isBookingLinkExpired;
//# sourceMappingURL=publicToken.js.map