...
const parseDay = (input) => {
    if (!(0, exports.isDateKey)(input.date))
        throw (0, taskErrors_1.taskBadRequest)('DAILY_REPORT_DATE_INVALID', 'Datum ungültig (YYYY-MM-DD).');
    ...
    if (!from || !to || Number.isNaN(from.getTime()) || Number.isNaN(to.getTime())
        ...
        throw (0, taskErrors_1.taskBadRequest)('DAILY_REPORT_RANGE_INVALID', 'Tagesgrenzen ungültig (from/to).');
    }
    return { date: input.date, from, to };
};
/** Zeit je Aufgabe im Tagesfenster — laufende Messungen zählen bis jetzt. */
const loadTaskTimes = async (tenantId, employeeId, from, to) => {
    const now = new Date();
    ...
    for (const row of rows) {
        ...
        if (!startedAt)
            continue;
        const end = (0, taskRows_1.rawDate)(row.endedAt) ?? now;
...
const toItems = (value) => (Array.isArray(value) ? value : []).filter((item) => typeof item === 'string');
const toTaskTimes = (value) => (Array.isArray(value) ? value : []).flatMap((entry) => {
...
const getMyDailyReport = async (actor, query) => {
    ...
    const weekStart = (0, exports.isDateKey)(query.weekStart) ? query.weekStart : day.date;
...
exports.getMyDailyReport = getMyDailyReport;
...
exports.DEFAULT_DAILY_REPORT_TIME = '16:00';
exports.DEFAULT_DAILY_REPORT_END_TIME = '17:00';
...
const getDailyReportSetting = async (tenantId) => {
    const rows = await prisma_client_1.default.$queryRaw(client_1.Prisma.sql `
    ...
    const promptTime = TIME_OF_DAY.test(start) ? start : exports.DEFAULT_DAILY_REPORT_TIME;
    // Altes Ende vor einem später gestellten Beginn: bis Tagesende offen.
    const fallbackEnd = toMinutes(exports.DEFAULT_DAILY_REPORT_END_TIME) > toMinutes(promptTime) ? exports.DEFAULT_DAILY_REPORT_END_TIME : '23:59';
    const endTime = TIME_OF_DAY.test(end) && toMinutes(end) > toMinutes(promptTime) ? end : fallbackEnd;
    return { promptTime, endTime };
};
exports.getDailyReportSetting = getDailyReportSetting;
const saveDailyReportSetting = async (actor, input) => {
    ...
    const endTime = (input.endTime ?? exports.DEFAULT_DAILY_REPORT_END_TIME).trim();
    if (!TIME_OF_DAY.test(promptTime) || !TIME_OF_DAY.test(endTime)) {
        throw (0, taskErrors_1.taskBadRequest)('DAILY_REPORT_TIME_INVALID', 'Uhrzeit ungültig (HH:MM).');
    }
    if (toMinutes(endTime) <= toMinutes(promptTime)) {
        throw (0, taskErrors_1.taskBadRequest)('DAILY_REPORT_WINDOW_INVALID', 'Das Ende muss nach dem Beginn liegen.');
    }
    const now = new Date();
    await prisma_client_1.default.$executeRaw(client_1.Prisma.sql `
        ...
        VALUES (${(0, nanoid_1.nanoid)(16)}, ${actor.tenantId}, ${promptTime}, ${endTime}, ${now}, ${now})
        ON DUPLICATE KEY UPDATE promptTime = ${promptTime}, endTime = ${endTime}, updatedAt = ${now}
    `);
    return { promptTime, endTime };
};
exports.saveDailyReportSetting = saveDailyReportSetting;
...
const assertInReportWindow = async (tenantId, day) => {
    const setting = await (0, exports.getDailyReportSetting)(tenantId);
    const now = Date.now();
    ...
    if (now >= day.to.getTime()
        ...
        throw (0, taskErrors_1.taskBadRequest)('DAILY_REPORT_WINDOW_CLOSED', 'Der Rapport kann nur im Zeitfenster geschrieben werden.', {
