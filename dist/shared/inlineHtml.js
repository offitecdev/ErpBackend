...
exports.inlineHtmlToText = void 0;
...
const decodeEntities = (value) => value.replace(/&(#[xX][0-9a-fA-F]+|#[0-9]+|[a-zA-Z]+);?/g, (match, body) => {
    if (body.startsWith('#')) {
...
function sanitizeInlineHtml(input, maxChars = 20_000) {
    ...
    while (index < source.length) {
        ...
        if (char === '<') {
            ...
            if (COMMENT_RE.test(source)) {
                index = COMMENT_RE.lastIndex;
                continue;
            }
            DROP_RE.lastIndex = index;
            if (DROP_RE.test(source)) {
                index = DROP_RE.lastIndex;
                continue;
            }
            ...
            if (closing) {
                ...
                if (at < 0)
                    continue; // loser Schliesser
                while (open.length > at)
                    out += `</${open.pop()}>`;
                continue;
            }
            if (open.length >= MAX_NESTING)
                continue;
            if (name === 'a') {
...
const inlineHtmlToText = (html) => decodeEntities(String(html ?? '').replace(/<br\s*\/?>/gi, '\n').replace(/<[^>]*>/g, ''))
    .replace(/\u00a0/g, ' ');
exports.inlineHtmlToText = inlineHtmlToText;
