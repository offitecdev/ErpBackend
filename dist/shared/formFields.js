...
exports.isFormValueEmpty = exports.CONDITION_OPERATORS = exports.NUMERIC_FIELD_TYPES = exports.FORM_FIELD_TYPES = void 0;
exports.normalizeFormFields = normalizeFormFields;
exports.computeFieldVisibility = computeFieldVisibility;
...
exports.NUMERIC_FIELD_TYPES = new Set([
...
function normalizeFormFields(raw) {
    ...
    for (const item of raw) {
        ...
        if (!exports.FORM_FIELD_TYPES.includes(type))
            continue;
        const label = str(item.label, 300);
        if (!label)
            continue;
        const field = {
            id: str(item.id, 40) || (0, nanoid_1.nanoid)(8),
            type,
            label,
        };
        if (item.required && type !== 'SECTION')
            field.required = true;
        ...
        if (help)
            field.help = help;
        if (type === 'TEXT' && item.multiline)
            field.multiline = true;
        if (type === 'SELECT') {
            const options = Array.isArray(item.options) ? item.options : [];
            field.options = options
                .map((option) => ({
                id: str(option?.id, 40) || (0, nanoid_1.nanoid)(6),
        ...
        if (condition && typeof condition === 'object') {
            ...
            if (fieldId && exports.CONDITION_OPERATORS.includes(operator)) {
                field.visibleWhen = { fieldId, operator };
                const value = str(condition.value, 200);
                if (value)
                    field.visibleWhen.value = value;
            }
        }
        fields.push(field);
    }
...
const isFormValueEmpty = (field, value) => {
    if (field.type === 'CHECKBOX')
        return value !== true;
    return isBlank(value);
};
exports.isFormValueEmpty = isFormValueEmpty;
 ...
 */
function computeFieldVisibility(fields, values) {
    ...
    for (const field of fields) {
        ...
        switch (condition.operator) {
            case 'EQUALS':
                ok = comparable(source, raw) === expected;
                break;
            case 'NOT_EQUALS':
                ok = comparable(source, raw) !== expected;
                break;
            ...
            case 'IS_NOT_CHECKED':
                ...
                break;
            case 'NOT_EMPTY':
                ok = !(0, exports.isFormValueEmpty)(source, raw);
                break;
            case 'EMPTY':
                ok = (0, exports.isFormValueEmpty)(source, raw);
                break;
            case 'GREATER_THAN': {
                ...
                ok = left !== null && right !== null && left > right;
                break;
            }
            case 'LESS_THAN': {
                const left = asNumber(raw);
 ...
 */
function sanitizeFormValues(fields, raw) {
    ...
    for (const field of fields) {
        ...
        switch (field.type) {
            ...
            case 'SELECT': {
                ...
                if (id && (field.options || []).some((option) => option.id === id))
                    out[field.id] = id;
                break;
            }
            case 'PHOTO': {
                if (!Array.isArray(value))
                    break;
                const photos = value
                    .map((photo) => (typeof photo === 'string' ? { dataUrl: photo } : photo))
                    .filter((photo) => photo && typeof photo.dataUrl === 'string' && photo.dataUrl.startsWith('data:image/'))
                    .slice(0, 30)
                    .map((photo) => ({ dataUrl: photo.dataUrl, ...(photo.caption ? { caption: str(photo.caption, 300) } : {}) }));
                if (photos.length)
                    out[field.id] = photos;
                break;
            }
            case 'FILE': {
                if (!Array.isArray(value))
                    break;
                const files = value
                    .filter((file) => file && typeof file.dataUrl === 'string' && file.dataUrl.startsWith('data:'))
                    .slice(0, 20)
                    .map((file) => ({
            ...
            case 'SIGNATURE': {
                const sig = typeof value === 'string' ? { dataUrl: value } : value;
...
function missingRequiredFields(fields, values) {
    ...
    return fields
        ...
        .filter((field) => (0, exports.isFormValueEmpty)(field, values[field.id]))
        .map((field) => field.id);
}
