...
const withSpec = (sourceVersion) => `${sourceVersion ?? ''}#${THUMB_SPEC}`;
const hasCurrentSpec = (sourceVersion) => typeof sourceVersion === 'string' && sourceVersion.endsWith(`#${THUMB_SPEC}`);
...
const getArticleThumbnails = async (tenantId, articles) => {
    if (articles.length === 0)
        return [];
    const keyOf = (article) => `${tenantId}:${article.id}:${article.updatedAt.getTime()}:${THUMB_SPEC}`;
