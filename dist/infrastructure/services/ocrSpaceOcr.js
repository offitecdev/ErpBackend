...
exports.readTextWithOcr = exports.ocrConfigured = exports.OcrError = exports.OCR_MAX_BYTES = void 0;
 ...
 */
exports.OCR_MAX_BYTES = 1024 * 1024;
...
const ocrConfigured = () => API_KEY().length > 0;
exports.ocrConfigured = ocrConfigured;
/**
 ...
 */
const isKeyProblem = (message) => /api\s*key|apikey|not\s*valid|invalid|unauthor/i.test(message);
/** Das Kontingent ist aufgebraucht oder die Anfragen kommen zu schnell. */
const isQuotaProblem = (message) => /limit|quota|exceed|too many|upto maximum/i.test(message);
/** Die Fehlermeldung des Dienstes als EIN Satz — sie kommt mal als Text, mal als Liste. */
const errorTextOf = (payload) => {
 ...
 */
const readTextWithOcr = async (base64, mimeType = 'image/jpeg') => {
    const key = API_KEY();
    if (!key)
        throw new OcrError('Die Texterkennung ist nicht eingerichtet.', 'OCR_NOT_CONFIGURED', 503);
    ...
    try {
        ...
        throw new OcrError(timedOut ? 'Die Texterkennung hat zu lange gebraucht.' : 'Die Texterkennung ist nicht erreichbar.', timedOut ? 'OCR_TIMEOUT' : 'OCR_UNREACHABLE', 504);
    }
    ...
    const overlayLines = Array.isArray(first?.TextOverlay?.Lines) ? first.TextOverlay.Lines : [];
    const lines = overlayLines
        .map((line) => {
        const words = Array.isArray(line?.Words) ? line.Words : [];
        const text = String(line?.LineText ?? words.map((w) => w?.WordText ?? '').join(' ')).trim();
...
exports.readTextWithOcr = readTextWithOcr;
//# sourceMappingURL=ocrSpaceOcr.js.map