...
exports.startRefreshSessionCleanup = exports.pruneRefreshSessions = exports.rotateRefreshSession = exports.RefreshSessionError = exports.revokeAllRefreshSessions = exports.revokeRefreshSession = exports.revokeRefreshFamily = exports.startRefreshSession = exports.REUSE_GRACE_MS = exports.REFRESH_SESSION_TTL_MS = void 0;
...
exports.REFRESH_SESSION_TTL_MS = 30 * 24 * 60 * 60 * 1000;
/** Fenster, in dem ein zweiter Tausch desselben Tokens ein Wettlauf ist. */
exports.REUSE_GRACE_MS = 10_000;
const sessions = () => prisma_client_1.default.refreshSession;
const createRow = async (ref, employeeId, tenantId, context) => {
    await sessions().create({
        data: {
            ...
            employeeId,
            tenantId,
            expiresAt: new Date(Date.now() + exports.REFRESH_SESSION_TTL_MS),
...
const startRefreshSession = async (employeeId, tenantId, context = {}) => {
    try {
        return await createRow({ jti: (0, nanoid_1.nanoid)(), sid: (0, nanoid_1.nanoid)() }, employeeId, tenantId, context);
    }
    catch (error) {
        ...
        throw new AuthErrors_1.PublicError('Oturum başlatılamadı. Lütfen daha sonra tekrar deneyin.');
    }
};
exports.startRefreshSession = startRefreshSession;
/** Jede noch offene Zeile einer Anmeldung entwerten. */
const revokeRefreshFamily = async (familyId, reason) => {
...
const revokeRefreshSession = async (jti, reason) => {
    await sessions().updateMany({
 ...
 */
const revokeAllRefreshSessions = async (employeeId, reason = 'account') => {
    await sessions().updateMany({
        where: { employeeId, revokedAt: null },
        data: { revokedAt: new Date(), revokedReason: reason },
    });
};
exports.revokeAllRefreshSessions = revokeAllRefreshSessions;
class RefreshSessionError extends AuthErrors_1.PublicError {
}
exports.RefreshSessionError = RefreshSessionError;
/**
 ...
 */
const rotateRefreshSession = async (ref, employeeId, tenantId, context = {}) => {
    ...
    if (!row) {
        await (0, exports.revokeRefreshFamily)(ref.sid, 'reuse');
        throw invalid();
    }
    ...
    if (row.employeeId !== employeeId || row.familyId !== ref.sid) {
        await (0, exports.revokeRefreshFamily)(row.familyId, 'reuse');
        throw invalid();
    }
    ...
    if (row.revokedAt) {
        ...
        const raced = row.revokedReason === 'rotated'
            && row.replacedById
            && Date.now() - row.revokedAt.getTime() <= exports.REUSE_GRACE_MS;
        if (raced) {
    ...
    const next = { jti: (0, nanoid_1.nanoid)(), sid: row.familyId };
...
exports.rotateRefreshSession = rotateRefreshSession;
/**
...
const pruneRefreshSessions = async () => {
    const result = await sessions().deleteMany({ where: { expiresAt: { lt: new Date() } } });
    return result.count ?? 0;
};
exports.pruneRefreshSessions = pruneRefreshSessions;
const startRefreshSessionCleanup = () => {
    const run = () => {
        (0, exports.pruneRefreshSessions)()
            .then((count) => { if (count)
            console.log(`[RefreshSession] ${count} abgelaufene Sitzungen abgeräumt.`); })
...
exports.startRefreshSessionCleanup = startRefreshSessionCleanup;
//# sourceMappingURL=RefreshSessionService.js.map