...
class SmtpMailService {
    async send(settings, mail) {
        ...
        let socket = settings.smtpSecure
            ? tls_1.default.connect(settings.smtpPort, settings.smtpHost, { servername: settings.smtpHost })
            : net_1.default.connect(settings.smtpPort, settings.smtpHost);
        ...
        const write = async (command, expected) => {
        ...
        if (!settings.smtpSecure && settings.smtpPort === 587) {
            ...
            socket = tls_1.default.connect({ socket, servername: settings.smtpHost });
            await write(`EHLO offitec-erp.local`, [250]);
        }
        ...
        const code = Number(response.slice(0, 3));
        if (code !== 250)
            throw new Error(`SMTP gonderim hatasi: ${response.trim()}`);
