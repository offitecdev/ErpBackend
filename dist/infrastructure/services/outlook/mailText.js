...
exports.stripHeaderValue = stripHeaderValue;
...
const sanitizeMailHtml = (html) => html
    ...
    if (lower === "a") {
        ...
        if (value && /^(https?:\/\/|mailto:)/i.test(value))
            safeAttrs += ` href="${value.replace(/"/g, "")}"`;
    }
    ...
    if (styleValue) {
        const kept = styleValue
            .split(";")
            .map((rule) => rule.trim())
            .filter((rule) => /^(color|font-size|font-weight|text-decoration)\s*:\s*[a-z0-9#(),.%\s-]+$/i.test(rule))
            .join("; ");
        if (kept)
...
exports.sanitizeMailHtml = sanitizeMailHtml;
...
exports.MAX_BODY_CHARS = 60_000;
const clampBody = (text) => {
    if (!text)
        return null;
...
const parseParty = (raw) => {
    const value = (0, exports.stripHeaderValue)(raw);
    if (!value)
        return null;
...
exports.parseParty = parseParty;
//# sourceMappingURL=mailText.js.map