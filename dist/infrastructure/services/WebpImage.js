...
exports.isConvertibleImageType = exports.WEBP_CONTENT_TYPE = void 0;
exports.toWebpImage = toWebpImage;
...
const isConvertibleImageType = (contentType) => CONVERTIBLE_CONTENT_TYPES.has(String(contentType || '').toLowerCase());
exports.isConvertibleImageType = isConvertibleImageType;
/**
 * Bild -> WebP. Alles andere kommt unveraendert zurueck; `converted` sagt,
 * was passiert ist.
 */
async function toWebpImage(body, contentType) {
    const unchanged = { body, contentType, converted: false };
    if (!Buffer.isBuffer(body) || body.length === 0)
        return unchanged;
    if (!(0, exports.isConvertibleImageType)(contentType))
        return unchanged;
    try {
        const meta = await (0, sharp_1.default)(body, { failOn: 'none' }).metadata();
        ...
        if (!best)
            return unchanged;
        return { body: best, contentType: exports.WEBP_CONTENT_TYPE, converted: true };
    }
    catch {
...
async function encode(body, animated, options) {
    try {
        const pipeline = (0, sharp_1.default)(body, { failOn: 'none', animated });
...
//# sourceMappingURL=WebpImage.js.map