...
exports.calendarEventsFromTnef = exports.parseTnef = exports.isTnef = void 0;
...
const keyOfTag = (id) => `tag:${id.toString(16).padStart(4, "0")}`;
const keyOfLid = (guid, lid) => `${guid}:${lid.toString(16).padStart(4, "0")}`;
const keyOfName = (guid, name) => `${guid}:${name.toLowerCase()}`;
const pad4 = (length) => (length + 3) & ~3;
/** Windows FILETIME (100-ns-Schritte seit 1601) → Date. */
const fileTimeToDate = (buffer, offset) => {
...
const appTimeToDate = (days) => {
    if (!Number.isFinite(days) || !days)
        return null;
    return new Date(Date.UTC(1899, 11, 30) + days * 86_400_000);
};
const guidOf = (buffer, offset) => {
    const hex = (start, length) => buffer.subarray(offset + start, offset + start + length).toString("hex");
    const swap = (value) => value.match(/../g).reverse().join("");
 ...
 */
const decodeString8 = (bytes, codepage) => {
 ...
 */
const readPropertyBag = (buffer, start, codepage) => {
    const bag = new Map();
        ...
        for (let v = 0; v < valueCount && offset <= buffer.length; v += 1) {
            switch (baseType) {
                ...
                case PT.R4:
                    ...
                    break;
                case PT.BOOLEAN:
                    values.push(buffer.readUInt16LE(offset) !== 0);
                    offset += 4;
                    break;
                case PT.DOUBLE:
                    ...
                    break;
                case PT.APPTIME:
                    values.push(appTimeToDate(buffer.readDoubleLE(offset)));
                    offset += 8;
                    break;
                case PT.CURRENCY:
                case PT.I8:
                    ...
                    break;
                case PT.SYSTIME:
                    values.push(fileTimeToDate(buffer, offset));
                    offset += 8;
                    break;
                case PT.CLSID:
                    values.push(guidOf(buffer, offset));
                    offset += 16;
                    break;
                case PT.STRING8:
                case PT.UNICODE:
                ...
                case PT.OBJECT: {
                    ...
                    if (baseType === PT.OBJECT && data.length >= 16)
                        data = data.subarray(16);
                    values.push(baseType === PT.STRING8 ? decodeString8(data, codepage)
...
const first = (bag, key) => bag?.get(key)?.values[0] ?? null;
const text = (bag, key) => {
...
const flag = (bag, key) => first(bag, key) === true;
const number = (bag, key) => {
    const value = first(bag, key);
...
const isTnef = (buffer) => buffer.length >= 6 && buffer.readUInt32LE(0) === TNEF_SIGNATURE;
exports.isTnef = isTnef;
/**
 ...
 */
const parseTnef = (buffer) => {
    if (!(0, exports.isTnef)(buffer))
        return null;
    const message = {
    ...
    while (offset + 9 <= buffer.length) {
        ...
        if (level === LEVEL_ATTACHMENT) {
            ...
            if (id === ATT.ATTACH_RENDDATA || !current) {
                ...
                if (id === ATT.ATTACH_RENDDATA)
                    continue;
            }
            if (id === ATT.ATTACH_DATA)
                current.data = Buffer.from(data);
            else if (id === ATT.ATTACH_TITLE)
                current.filename = decodeString8(data, message.codepage) || current.filename;
        ...
        switch (id) {
            case ATT.OEM_CODEPAGE:
                if (data.length >= 4)
                    message.codepage = data.readUInt32LE(0) || 1252;
                break;
            ...
            case ATT.MAPI_PROPS: {
                const { bag } = readPropertyBag(data, 0, message.codepage);
                for (const [key, property] of bag)
                    message.props.set(key, property);
                break;
            }
 ...
 */
const methodOfClass = (messageClass) => {
    const value = String(messageClass || "").toLowerCase();
    if (!value)
        return null;
    if (/schedule\.meeting\.request|schedule\.mtgreq/.test(value))
        return "REQUEST";
    if (/schedule\.meeting\.canceled|schedule\.mtgcncl/.test(value))
        return "CANCEL";
    if (/schedule\.meeting\.resp|schedule\.mtgresp/.test(value))
        return "REPLY";
    // Ein blosser Kalendereintrag (weitergeleitet, exportiert): behandelt wie
    // eine Einladung — er beschreibt denselben Termin.
    if (/^ipm\.appointment/.test(value))
        return "REQUEST";
    return null;
};
const addressOf = (bag) => {
...
const onlineUrlOf = (props) => {
    for (const property of props.values()) {
        if (!/skypeteamsmeetingurl|onlinemeetingexternallink|onlinemeetingconflink/.test(property.key))
            continue;
 ...
 */
const eventFromMessage = (message) => {
...
const looksLikeCalendarAttachment = (attachment) => /\.ics$/i.test(String(attachment.filename || "")) || /text\/calendar/i.test(String(attachment.mimeType || ""));
/**
 ...
 */
const calendarEventsFromTnef = (buffer) => {
    const message = (0, exports.parseTnef)(buffer);
    if (!message)
        return [];
    const fromAttachments = message.attachments
        .filter((attachment) => attachment.data && looksLikeCalendarAttachment(attachment))
        .flatMap((attachment) => (0, calendarInvite_1.parseCalendarObjects)(attachment.data.toString("utf8")));
    if (fromAttachments.length)
        return fromAttachments;
    const own = eventFromMessage(message);
    return own ? [own] : [];
};
exports.calendarEventsFromTnef = calendarEventsFromTnef;
//# sourceMappingURL=tnef.js.map