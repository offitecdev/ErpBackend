 ...
 * eigenes Zeichen.
 */
const kindIconCid = (kind) => kind === "TASK" ? mailBrand_1.BRAND_ICON_TASK_CID : mailBrand_1.BRAND_ICON_APPOINTMENT_CID;
...
const buildInviteText = (input) => {
    ...
    const rows = [
        input.hideDate ? null : `${kind === "TASK" ? words.due : words.date}: ${(0, exports.formatInviteDate)(input.start, language)}`,
        kind === "TASK" || input.hideDate ? null : `${words.time}: ${(0, exports.formatInviteTime)(input.start, input.end, language)}`,
...
const buildInviteHtml = (input) => {
    ...
    const softBox = (accent, background, inner) => `<div style="margin-top:14px;padding:11px 14px;background:${background};border-left:3px solid ${accent};border-radius:10px;">${inner}</div>`;
    return `<!DOCTYPE html>
<html lang="${language}">
<head>
<meta charset="utf-8" />
...
<tr><td align="center" style="padding:28px 16px 36px;">
    ...
    <tr><td style="padding:20px 24px 16px;">
        ...
        <tr>
            <td style="vertical-align:middle;width:36px;">
                <img src="cid:${mailBrand_1.BRAND_LOGO_CID}" width="34" height="34" alt="" style="display:block;width:34px;height:34px;border:0;border-radius:17px;" />
            </td>
            <td style="${FONT}vertical-align:middle;padding-left:11px;">
                <div style="font-size:14.5px;font-weight:700;color:${mailBrand_1.BRAND_NAVY};letter-spacing:.01em;">${escapeHtml(words.brand)}</div>
    ...
    <tr><td style="padding:0;font-size:0;line-height:0;">
        <img src="cid:${mailBrand_1.BRAND_WAVE_CID}" width="${WIDTH}" height="62" alt="" style="display:block;width:100%;max-width:${WIDTH}px;height:auto;border:0;" />
    </td></tr>
    ...
    <tr><td align="center" style="padding:14px 26px 0;">
        ...
        <div style="${FONT}font-size:20px;line-height:1.3;font-weight:700;color:${mailBrand_1.BRAND_NAVY};margin-top:9px;${cancelled ? "text-decoration:line-through;color:#64748b;" : ""}">${escapeHtml(input.summary)}</div>
    </td></tr>
    ...
    <tr><td style="padding:12px 26px 0;">
        ...
        ? softBox(mailBrand_1.BRAND_NAVY, "#f1f5fd", `<div style="${FONT}font-size:10px;font-weight:700;letter-spacing:.14em;text-transform:uppercase;color:#8b93a7;">${escapeHtml(words.attachments)}</div>` +
