...
type BatchResult = { status: number; body: unknown };

const parseGet = (raw: string): string | null => {
    ...
    if (url.host !== 'batch.local' || !ALLOWED_PATHS.has(url.pathname)) return null;
...
router.get('/', requireAuth, async (req, res) => {
    const rawGets = ([] as unknown[]).concat(req.query.get ?? []).map(String);
    ...
    for (const raw of rawGets) {
        ...
        if (!parsed) return res.status(400).json({ error: `Pfad nicht erlaubt: ${raw.slice(0, 80)}` });
        gets.push(parsed);
    }
