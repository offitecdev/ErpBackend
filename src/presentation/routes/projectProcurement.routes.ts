...
import { readTenantCompanyType } from '../../shared/companyType';
import { onProjectCreated } from '../../shared/projectEvents';

const router = Router();

type ProcurementMode = 'PURCHASE' | 'PRODUCTION' | 'EMPTY';
 ...
 */
const isProductLine = (row: any): boolean => {
    const type = String(row?.rowType ?? '').trim().toUpperCase();
    if (type === 'DESCRIPTION' || type === 'TITLE' || type === 'SECTION') return false;
    if (type === 'PRODUCT' || type === 'CUSTOM') return true;
...
const byDisplayOrder = (a: any, b: any): number => {
...
const round3 = (value: number): number => Math.round((Number(value) || 0) * 1000) / 1000;

const plain = (value: unknown, max = 500): string => String(value ?? '')
    .replace(/<[^>]+>/g, ' ')
    .replace(/&nbsp;/g, ' ')
    .replace(/&amp;/g, '&')
    .replace(/\s+/g, ' ')
...
const projectLabelOf = (project: { projectNumber?: string | null; projectName?: string | null }): string => {
    const number = String(project.projectNumber ?? '').trim();
    const name = String(project.projectName ?? '').trim();
    if (number && name && name !== number) return `${number} · ${name}`.slice(0, 191);
 ...
 */
const resolveProducer = async (tenantId: string) => {
    ...
    const settings = await (prisma as any).productionTransferSetting.findMany({
        where: { tenantId: { in: candidates.map((c: any) => c.id) } },
        select: { tenantId: true, sourceTenantIds: true },
    });
    const chosen = settings.find((row: any) => Array.isArray(row.sourceTenantIds) && row.sourceTenantIds.includes(tenantId));
    return candidates.find((c: any) => c.id === chosen?.tenantId) ?? candidates[0];
};

/** Üretim şirketi, proje şirketinin tedarikçi listesinde (adıyla, bir kez). */
const producerSupplier = async (tenantId: string, producer: any) => {
    const companyName = String(producer.tenantName || '').trim().slice(0, 191) || 'Produktion';
    const address = composeAddressSnapshot({
    ...
    return { supplier, address: supplierAddressSnapshot(supplier) ?? address };
};

interface PositionOrderRef {
    id: string;
    referenceNumber: string;
    status: string;
    /** Siparişin tedarikçisi — boşsa henüz seçilmedi (Türsüzler'den açılan sipariş). */
    supplierName: string;
    quantity: number;
}
...
interface PositionInfo {
    ...
    name: string;
    unit: string | null;
    /** Talep edilen miktar (pozisyon miktarı × ürün çarpanı). */
    requested: number;
    ...
    unitCost: number;
    /** Bu satır için depoda KALAN stok (aynı ürünün önceki satırları payını aldıktan sonra). */
    stock: number | null;
    /** Talebin stoktan karşılanan kısmı. */
    fromStock: number;
    /** Siparişlerde (fiyat talebi değil) bu pozisyon için duran miktar. */
    ordered: number;
    /** Fiyat taleplerinde duran miktar — işlemde sayılır, yeniden sipariş edilmez. */
    inRequest: number;
    /** Hâlâ sipariş edilmesi gereken: talep − sipariş − fiyat talebi − stoktan karşılanan. */
    missing: number;
 ...
 */
const isOrderedByHand = (info: PositionInfo): boolean =>
    info.mode === 'PURCHASE' && !(info.articleKind === 'RESALE' && Boolean(info.supplier));

/** İş kuralı hatası — arayüz `code`'dan ne yapacağını bilir. */
class ProcurementError extends Error {
    constructor(public code: string, message: string, public status = 409, public extra: Record<string, unknown> = {}) {
        super(message);
    }
}

const sendProcurementError = (res: any, error: any) => {
    if (error instanceof ProcurementError) {
        return res.status(error.status).json({ error: error.message, code: error.code, ...error.extra });
    }
    return sendPurchaseOrderError(res, error);
};
 ...
 */
const buildProcurement = async (tenantId: string, projectId: string) => {
    ...
    const lines = (positions as any[]).filter(isProductLine).sort(byDisplayOrder);
    const articleOf = (row: any): { id: string | null; multiplier: number } => {
        if (row.sourceArticleId) return { id: String(row.sourceArticleId), multiplier: 1 };
    ...
    const articleIds = [...new Set(lines.map((row) => articleOf(row).id).filter(Boolean) as string[])];
    ...
    const [articles, balances, links] = articleIds.length
        ? await Promise.all([
            ...
            (prisma as any).stockBalance.groupBy({
                by: ['articleId'],
    ...
    const articleById = new Map<string, any>((articles as any[]).map((a) => [a.id, a]));
    // Kullanılabilir stok = mevcut − rezerve; eksiye düşmüş bakiye 0 sayılır.
    const stockPool = new Map<string, number>((balances as any[]).map((b) => [
        String(b.articleId),
        Math.max(0, round3((Number(b._sum?.currentQuantity) || 0) - (Number(b._sum?.reservedQuantity) || 0))),
    ]));
    const linkByArticle = new Map<string, any>();
    for (const link of links as any[]) if (!linkByArticle.has(link.articleId)) linkByArticle.set(link.articleId, link);

    const supplierIds = new Set<string>();
    for (const article of articles as any[]) {
        const id = article.defaultSupplierId || linkByArticle.get(article.id)?.supplierId;
        if (id) supplierIds.add(String(id));
    }
    ...
    const inRequestByPosition = new Map<string, number>();
    const ordersByPosition = new Map<string, PositionOrderRef[]>();
    /** Siparişin bu projeye ait pozisyonları ve «elle açıldı» işareti (U-başlığı için). */
    const orderLines = new Map<string, { marked: boolean; positionIds: string[] }>();
    const orders = (orderRows as any[]).map((row) => {
        const items = parseItems(row.items);
        const own = items.filter((item) => poLineSource(item?.source)?.projectId === projectId);
        orderLines.set(row.id, {
            marked: own.some((item) => poLineSource(item.source)?.manual === true),
            positionIds: own.map((item) => poLineSource(item.source)?.positionId).filter(Boolean) as string[],
        });
        const status = String(row.status);
        for (const item of own) {
            const positionId = poLineSource(item.source)?.positionId;
            if (!positionId) continue;
            const quantity = Number(item.quantity) || 0;
            if (ORDER_STATUSES.has(status)) orderedByPosition.set(positionId, round3((orderedByPosition.get(positionId) ?? 0) + quantity));
            else if (REQUEST_STATUSES.has(status)) inRequestByPosition.set(positionId, round3((inRequestByPosition.get(positionId) ?? 0) + quantity));
            const list = ordersByPosition.get(positionId) ?? [];
            const existing = list.find((entry) => entry.id === row.id);
            if (existing) existing.quantity = round3(existing.quantity + quantity);
            else list.push({ id: row.id, referenceNumber: row.referenceNumber, status, supplierName: row.supplierName || '', quantity });
        ...
        return {
            ...
            production: own.some((item) => Boolean(poLineSource(item.source)?.producerTenantId)),
    ...
    const infos: PositionInfo[] = lines.map((row) => {
        ...
        const mode: ProcurementMode = isService
            ? 'EMPTY'
    ...
    for (const order of orders) {
        ...
        if (!own) continue;
        const known = own.positionIds.map((id) => infoById.get(id)).filter(Boolean) as PositionInfo[];
...
router.get(
    '/:projectId',
    requireAuth,
    requireAnyPermission(['inventory.view', 'projects.view']),
    async (req, res) => {
        try {
            const data = await buildProcurement(req.user!.tenantId, String(req.params.projectId));
            if (!data) return res.status(404).json({ error: 'Proje bulunamadı.', code: 'PROJECT_NOT_FOUND' });
 ...
 */
const lineFor = (info: PositionInfo, projectId: string, quantity: number, producerTenantId: string | null, manual = false) => {
    ...
    return {
        ...
        source: { projectId, positionId: info.positionId, producerTenantId, ...(manual ? { manual: true as const } : {}) },
    };
};
...
interface OrderTarget {
    ...
    /** Üretim şirketine giden siparişte o şirketin kimliği. */
    producerTenantId: string | null;
}

/** Üretim şirketi hedefi (tedarikçi kaydıyla); tanımlı değilse `null`. */
const producerTarget = async (tenantId: string): Promise<OrderTarget | null> => {
...
const supplierTarget = async (tenantId: string, supplierId: string): Promise<OrderTarget | null> => {
    const supplier = await (prisma as any).supplier.findFirst({ where: { id: supplierId, tenantId } });
    return supplier ? { supplier, supplierAddress: supplierAddressSnapshot(supplier), producerTenantId: null } : null;
};
...
const writeOrderLines = async (
...
): Promise<{ row: any; created: boolean; merged: boolean }> => {
    ...
    if (open) {
        ...
        for (const line of lines) {
            const same = items.find((item) => {
                const source = poLineSource(item?.source);
            ...
            if (same) {
                ...
                if (String(same.calcMode || 'DIRECT').toUpperCase() === 'DIRECT') same.lineTotal = round2(nextQty * unit);
                merged = true;
            } else {
                items.push(line);
            }
        ...
        const row = await (prisma as any).purchaseOrder.update({
            where: { id: open.id },
            data: {
                ...
                totalVat: purchaseOrderTotalVat(
                    vat, normalized.totalNet, Number(open.totalFees) || 0, normalized.totalVat,
                    normalized.items.map((it: any) => it.lineTotal),
                ),
    ...
    const normalized = normalizePurchaseOrderItems(lines);
        ...
        try {
            row = await (prisma as any).purchaseOrder.create({
                data: {
                    ...
                    tableColumns: standardTableColumnsJson('ORDER'),
                    ...
                    totalNet: normalized.totalNet,
                    totalGross: normalized.totalGross,
                    totalVat: purchaseOrderTotalVat(vat, normalized.totalNet, 0, normalized.totalVat, normalized.items.map((it: any) => it.lineTotal)),
                    totalFees: 0,
    ...
    if (!row) throw new ProcurementError('NUMBER_FAILED', 'Sipariş numarası üretilemedi, lütfen tekrar deneyin.', 400);
...
router.post(
    ...
    async (req, res) => {
        try {
            ...
            if (!data) throw new ProcurementError('PROJECT_NOT_FOUND', 'Proje bulunamadı.', 404);
            const info = data.positions.find((entry) => entry.positionId === positionId);
            if (!info) throw new ProcurementError('POSITION_NOT_FOUND', 'Pozisyon bulunamadı.', 404);
            if (info.mode === 'EMPTY') throw new ProcurementError('EMPTY_FORM', 'Bu pozisyon için boş sipariş formu açılır.');
            ...
            let target: OrderTarget | null;
            if (info.mode === 'PRODUCTION') {
                target = await producerTarget(tenantId);
                if (!target) throw new ProcurementError('PRODUCER_MISSING', 'Ayarlarda türü «Üretim» olan bir şirket tanımlı değil.');
            } else {
                target = info.supplier ? await supplierTarget(tenantId, info.supplier.id) : null;
                if (!target) throw new ProcurementError('SUPPLIER_MISSING', 'Ürünün varsayılan tedarikçisi yok.');
            }

            const result = await writeOrderLines(
                { tenantId, projectId, projectLabel: data.project.label, userId: req.user!.id },
                target,
                [lineFor(info, projectId, quantity, target.producerTenantId)],
                mode,
            );
            res.status(result.created ? 201 : 200).json({
                created: result.created,
                merged: result.merged,
                order: parsePurchaseOrderRow(result.row),
            });
        } catch (error: any) {
            sendProcurementError(res, error);
        }
...
const withProjectLock = async <T,>(key: string, run: () => Promise<T>): Promise<T> => {
    ...
    try {
        ...
        if (projectLocks.get(key) === next) projectLocks.delete(key);
    }
};

/** Otomatik siparişe girenler: tedarikçisi belli «Satın Alınacak» ve «Üretilecek». */
const isAutoOrdered = (info: PositionInfo): boolean =>
    (info.mode === 'PRODUCTION') || (info.mode === 'PURCHASE' && info.articleKind === 'RESALE' && Boolean(info.supplier));

/** Tablodaki grubun anahtarı — frontend `buildGroups` ile aynı: `P:<tedarikçi>` / `M:<üretim>`. */
const groupKeyOf = (info: PositionInfo): string | null => {
    if (!info.supplier) return null;
    return `${info.mode === 'PRODUCTION' ? 'M' : 'P'}:${info.supplier.id}`;
};

const runAutoOrder = async (ctx: WriteContext, positions: PositionInfo[]) => {
    const groups = new Map<string, { target: OrderTarget; lines: Array<ReturnType<typeof lineFor>> }>();
    let producer: OrderTarget | null | undefined;
    const suppliers = new Map<string, OrderTarget | null>();

    for (const info of positions) {
        if (!isAutoOrdered(info) || !(info.missing > 0)) continue;
        let target: OrderTarget | null = null;
        if (info.mode === 'PRODUCTION') {
            if (producer === undefined) producer = await producerTarget(ctx.tenantId);
            target = producer;
        } else if (info.supplier) {
            if (!suppliers.has(info.supplier.id)) suppliers.set(info.supplier.id, await supplierTarget(ctx.tenantId, info.supplier.id));
    ...
    const written: Array<{ id: string; referenceNumber: string; supplierName: string; created: boolean; lineCount: number }> = [];
 ...
 */
export const autoOrderProject = async (tenantId: string, projectId: string, userId: string) => {
    if (await readTenantCompanyType(tenantId) !== 'PROJECT') return [];
 ...
 */
export const scheduleProjectAutoOrder = (tenantId: string, projectId: string, userId: string): void => {
    setImmediate(() => {
        autoOrderProject(tenantId, projectId, userId)
            .then((written) => {
                if (written.length) {
                    console.log('[procurement] auto orders for project', projectId, written.map((entry) => entry.referenceNumber).join(', '));
                }
            })
            .catch((error: any) => console.warn('[procurement] auto order failed', projectId, error?.message));
    });
};

// Proje oluşturan akışlar olayı yayınlar; sipariş modülü burada dinler.
onProjectCreated(({ tenantId, projectId, userId }) => scheduleProjectAutoOrder(tenantId, projectId, userId));
...
router.post(
    ...
    async (req, res) => {
        try {
            ...
            if (!key) throw new ProcurementError('GROUP_REQUIRED', 'Tedarikçi grubu seçilmedi.', 400);
            ...
            if (!result) throw new ProcurementError('SUPPLIER_MISSING', 'Tedarikçi bulunamadı.');
            const row = await (prisma as any).purchaseOrder.findFirst({ where: { id: result.id, tenantId } });
            res.status(result.created ? 201 : 200).json({
                created: result.created,
                merged: !result.created,
                order: parsePurchaseOrderRow(row),
            });
        } catch (error: any) {
            sendProcurementError(res, error);
        }
    ...
    async (req, res) => {
        try {
            ...
            const wanted = new Set<string>(
                (Array.isArray(req.body?.positionIds) ? req.body.positionIds : [])
                    .map((value: unknown) => String(value ?? '').trim())
                    .filter(Boolean),
            );
            if (!wanted.size) throw new ProcurementError('NOTHING_SELECTED', 'Sipariş için en az bir ürün işaretleyin.', 400);
            const result = await withProjectLock(`${tenantId}:${projectId}`, async () => {
                const data = await buildProcurement(tenantId, projectId);
                if (!data) throw new ProcurementError('PROJECT_NOT_FOUND', 'Proje bulunamadı.', 404);
                const chosen = data.positions.filter((info) => wanted.has(info.positionId) && info.mode !== 'EMPTY');
                if (!chosen.length) throw new ProcurementError('NOTHING_SELECTED', 'İşaretlenen ürün bulunamadı.', 400);
                ...
                const written = await writeOrderLines(
                    { tenantId, projectId, projectLabel: data.project.label, userId: req.user!.id },
                    null,
                    lines,
                    'NEW',
            ...
            res.status(201).json({ created: true, merged: false, order: parsePurchaseOrderRow(result.row) });
        } catch (error: any) {
            sendProcurementError(res, error);
        }
    },
