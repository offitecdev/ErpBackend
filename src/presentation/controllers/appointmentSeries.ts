...
import { findTechnicianScheduleConflict } from './technicianSchedule';
...
/**
 * Ein geplanter Tag. Er DARF über Mitternacht laufen (Vorgabe Samet,
 * 24.08.2026: «eine Karte über zwei Tage soll EINE Karte bleiben, nicht zwei
 * Termine»): eine Nachtmontage von 20:00 bis 02:00 ist EIN Einsatztag mit
 * einer durchgehenden Arbeitszeit — und genau eine Zeile, ein Rapport, eine
 * Überstundenrechnung. Was ihn begrenzt, ist nicht der Kalendertag, sondern
 * die Länge: mehr als 24 Stunden am Stück ist keine Schicht mehr.
 */
export interface PlannedDay {
    startTime: Date;
    endTime: Date;
    /** Nur beim Bearbeiten einer Serie: die Zeile, die dieser Tag fortschreibt. */
 ...
 */
export const parseAppointmentDays = (body: any): PlannedDay[] => {
    ...
    if (raw.length > MAX_SERIES_DAYS) {
        throw fail(`Ein Einsatz umfasst höchstens ${MAX_SERIES_DAYS} Tage.`);
    }

    const days: PlannedDay[] = raw.map((entry: any) => {
    ...
    for (const day of days) {
        const key = localDayKey(day.startTime);
        if (seen.has(key)) throw fail(`Für den ${formatDay(day.startTime)} sind zwei Zeiten angegeben. Je Tag gilt eine Arbeitszeit.`);
        seen.add(key);
    }
 ...
 */
export const assertDaysAvailable = async (input: {
    tenantId: string;
    projectId: string;
    ...
    days: PlannedDay[];
    /** Zeilen, die zum Einsatz selbst gehören und sich deshalb nicht blockieren. */
 ...
 */
export const createSeries = async (tx: any, tenantId: string, coverNote?: string | null): Promise<string> => {
    const id = nanoid(10);
 ...
 */
export const ensureSeriesId = async (appointment: { id: string; tenantId: string; seriesId?: string | null }): Promise<string> => {
    if (appointment.seriesId) return appointment.seriesId;
    return await (prisma as any).$transaction(async (tx: any) => {
        const seriesId = await createSeries(tx, appointment.tenantId);
...
export const seriesDays = async (seriesId: string, tenantId: string) =>
 ...
 */
export const renumberSeries = async (tx: any, seriesId: string): Promise<void> => {
    const days = await tx.appointment.findMany({
        where: { seriesId },
        orderBy: { startTime: "asc" },
        select: { id: true, dayIndex: true },
    });
    await Promise.all(days.map((day: any, index: number) => (
        day.dayIndex === index
            ? Promise.resolve(null)
...
export const DOCUMENT_LIMIT_BYTES = 12 * 1024 * 1024;
/** Und je Einsatz höchstens 60 MB, damit die Zeile ladbar bleibt. */
export const SERIES_DOCUMENT_LIMIT_BYTES = 60 * 1024 * 1024;
 ...
 */
export const sanitizeDocumentUpload = (raw: any, file?: { originalname?: string; mimetype?: string; buffer: Buffer }): DocumentUpload => {
    const fileName = String(file?.originalname || raw?.fileName || "").trim().replace(/[\r\n]/g, " ").slice(0, 180);
    if (!fileName) throw fail("Eine Unterlage braucht einen Dateinamen.");

    if (file?.buffer) {
        const contentType = String(file.mimetype || raw?.contentType || "").trim().toLowerCase();
        if (!DOCUMENT_TYPES.has(contentType)) throw fail("Als Unterlage sind PDF und Bilder möglich.");
        if (!file.buffer.length) throw fail("Die Unterlage ist leer.");
        if (file.buffer.length > DOCUMENT_LIMIT_BYTES) {
            throw fail(`Eine Unterlage darf höchstens ${Math.round(DOCUMENT_LIMIT_BYTES / (1024 * 1024))} MB gross sein.`);
        }
        return { fileName, contentType, sizeBytes: file.buffer.length, body: file.buffer };
    }

    const rawData = String(raw?.data ?? "").trim();
    if (!rawData) throw fail("Eine Unterlage braucht einen Inhalt.");
    ...
    if (!sizeBytes) throw fail("Die Unterlage ist leer.");
    if (sizeBytes > DOCUMENT_LIMIT_BYTES) {
        throw fail(`Eine Unterlage darf höchstens ${Math.round(DOCUMENT_LIMIT_BYTES / (1024 * 1024))} MB gross sein.`);
    }
