...
const tenderGrossSql = (tenantIds: string[]) => Prisma.sql`
    SELECT COALESCE(SUM(
        sub.gross
    ...
    JOIN (
        SELECT p.tenderId AS tenderId,
            SUM(
                ...
                END)
                * (1 + (CASE
                    WHEN COALESCE(p.taxRate, 0) = 0 THEN ${DEFAULT_VAT}
                    ELSE p.taxRate
                END) / 100)
            ) AS gross
        FROM Position p
    ...
    WHERE t.tenantId IN (${Prisma.join(tenantIds)})
`;
...
export class DashboardController {
    ...
    async getSummary(req: Request, res: Response): Promise<void> {
        try {
            ...
            const [
                ...
                prisma.customer.count({ where: { ...scoped, isActive: true } }),
                prisma.tender.count({ where: scoped }),
                prisma.project.count({ where: scoped }),
                prisma.project.count({ where: { ...scoped, status: 'ACTIVE' } }),
                prisma.tender.count({ where: { ...scoped, salesOrder: { isNot: null } } }),
                prisma.tender.count({ where: { ...scoped, salesOrder: { is: { projectId: { not: null } } } } }),
                prisma.tender.count({ where: { ...scoped, salesOrder: { is: { orderType: 'INVOICE' } } } }),
            ...
            const bucketTotal = (bucket: Set<string>) => round2(
                orderCountsByType
    ...
    async getCharts(req: Request, res: Response): Promise<void> {
        try {
            ...
            const tenantIn = Prisma.join(tenantIds);
            ...
            const [tenderMonths, orderMonths, invoiceMonths, tendersByStatus, projectsByStatus, invoicesByKind, customersByStatus] = await Promise.all([
                prisma.$queryRaw<Array<{ month: string; count: bigint }>>(Prisma.sql`
                    ...
                    GROUP BY month
                `),
                prisma.$queryRaw<Array<{ month: string; count: bigint; total: number | null }>>(Prisma.sql`
                ...
                prisma.$queryRaw<Array<{ month: string; total: number | null }>>(Prisma.sql`
                    ...
                    GROUP BY month
                `),
                prisma.tender.groupBy({ by: ['status'], where: scoped, _count: { _all: true } }),
