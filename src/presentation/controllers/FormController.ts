 ...
 */
function contextCondition(context: FormContext, alias: string, opts: { hasTender: boolean } = { hasTender: true }): Prisma.Sql {
...
export class FormController {
    ...
    async listSubmissions(req: Request, res: Response) {
        try {
            ...
            for (const key of ['templateId', 'customerId', 'tenderId', 'salesOrderId', 'projectId', 'appointmentId'] as const) {
                const value = String(req.query[key] || '').trim();
                if (value) conditions.push(Prisma.sql`${Prisma.raw(`s.${key}`)} = ${value}`);
            }
            const search = String(req.query.search || '').trim();
            if (search) {
                const like = `%${search}%`;
    ...
    async getSubmission(req: Request, res: Response) {
        try {
            ...
            const heavy = await prisma.formSubmission.findUnique({
                where: { id: row.id },
