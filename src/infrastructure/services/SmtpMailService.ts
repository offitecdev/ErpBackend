...
const sniFor = (host: string) => (net.isIP(host) ? undefined : host);

const openSocket = (host: string, port: number, secure: boolean): Promise<SmtpSocket> =>
...
const upgradeToTls = (socket: SmtpSocket, host: string): Promise<tls.TLSSocket> =>
    new Promise((resolve, reject) => {
