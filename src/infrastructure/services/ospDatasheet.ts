...
export interface OspDatasheetSpecs {
    /** "227.3 kW" — Heiz- bzw. Kühlleistung, je nach Kategorie. */
    power?: string;
...
const flatKey = (key: string): string => key.toLowerCase().replace(/[\s_\-.]/g, '');
 ...
 */
const urlFieldScore = (key: string): number => {
    const flat = flatKey(key);
    // Der Weg zur Offerte/zum Projekt in der OSP — nicht das Datenblatt.
    if (/(proposal|offerlink|offerurl|projecturl|projectlink|permalink|weblink)/.test(flat)) return 0;
    let score = 0;
    if (flat.includes('datasheet') || flat.includes('datenblatt')) score += 100;
    if (flat.includes('pdf')) score += 80;
    if (flat.includes('report')) score += 40;
    if (flat.includes('document') || flat.includes('doc')) score += 30;
    if (flat.includes('attachment') || flat.includes('file')) score += 20;
    ...
    if (/(url|link|href|uri)$/.test(flat)) score += 5;
    return score;
};
...
const describeFailure = (error: any, url: string): string => {
    if (error?.name === 'TimeoutError' || error?.name === 'AbortError') {
        return `Datenblatt antwortet nicht (${FETCH_TIMEOUT_MS} ms überschritten): ${url}`;
    }
    const code = error?.cause?.code || error?.code;
    if (code === 'ENOTFOUND') return `Adresse des Datenblatts unbekannt (DNS): ${url}`;
    if (code === 'ECONNREFUSED') return `Verbindung zum Datenblatt abgewiesen: ${url}`;
 ...
 */
const labelled = (text: string, labels: string[], valuePattern: string): string | undefined => {
    for (const label of labels) {
        const pattern = new RegExp(
            String.raw`${label}\s*[:\-–]?\s*(?:\r?\n\s*)?(${valuePattern})`,
            'i',
        );
 ...
 */
const technologyLines = (text: string): string | undefined => {
    const wanted: RegExp[] = [
    ...
    for (const raw of text.split(/\r?\n/)) {
        ...
        if (!wanted.some((pattern) => pattern.test(line))) continue;
 ...
 */
export const parseDatasheetSpecs = (text: string): OspDatasheetSpecs => {
    ...
    if (cooling) { specs.power = cooling; specs.powerIsCooling = true; }
    else if (heating) { specs.power = heating; specs.powerIsCooling = false; }
    ...
    if (medium) specs.medium = medium;

    const technology = technologyLines(text);
    if (technology) specs.technology = technology;

    const sound1m = labelled(
        text,
        [String.raw`Schalldruck(?:pegel)?\s*(?:bei|in|@)?\s*1\s*m`, String.raw`Sound pressure\s*(?:at|@)?\s*1\s*m`],
        String.raw`${NUM}\s*dB\s*\(?A?\)?[^\n]{0,60}`,
    );
    if (sound1m) specs.sound1m = sound1m;

    const sound10m = labelled(
        text,
        [String.raw`Schalldruck(?:pegel)?\s*(?:bei|in|@)?\s*10\s*m`, String.raw`Sound pressure\s*(?:at|@)?\s*10\s*m`],
        String.raw`${NUM}\s*dB\s*\(?A?\)?[^\n]{0,60}`,
    );
    if (sound10m) specs.sound10m = sound10m;

    const dimensions = labelled(
        text,
        [String.raw`Abmessungen(?:\s*\(?\s*L\s*[x×]\s*B\s*[x×]\s*H\s*\)?)?`, 'Dimensions', 'Masse'],
        String.raw`(?:ca\.\s*)?${NUM}\s*(?:mm|m)?\s*[x×]\s*${NUM}\s*(?:mm|m)?\s*[x×]\s*${NUM}\s*(?:mm|m)`,
    );
    if (dimensions) specs.dimensions = dimensions;

    const weight = labelled(
        text,
        ['Betriebsgewicht', 'Gewicht', 'Operating weight', 'Leergewicht'],
        String.raw`(?:ca\.\s*)?${NUM}\s*kg`,
    );
 ...
 */
export const fetchOspDatasheet = async (
    ...
    if (!body.length) return { ok: false, error: 'Datenblatt ist leer.' };
    ...
    let file: string;
    try {
        file = await ospDatasheetStorage.store(tenantId, body, 'application/pdf');
    } catch (error: any) {
