...
import type { OspDatasheetSpecs } from './ospDatasheet';

/** Eine Zeile des Blattes: "Cooling Capacity kW 82.32". */
export interface DatasheetRow {
    label: string;
    unit: string | null;
    value: string;
    /** Die Zeile, wie sie im Blatt steht — für die lesbare Fassung. */
    raw: string;
}
...
export interface DatasheetSection {
    title: string;
    rows: DatasheetRow[];
    /** Ganze Sätze (Hinweise, Bedingungen) — nie eine Angabe. */
    notes: string[];
}

export interface DatasheetDocument {
    sections: DatasheetSection[];
    /** Alle Zeilen des Blattes, in der Reihenfolge des Blattes. */
    rows: DatasheetRow[];
}
 ...
 */
const splitRow = (line: string): DatasheetRow | null => {
    ...
    for (let index = words.length - 2; index >= 1; index -= 1) {
        ...
        if (!UNIT_TOKENS.includes(word)) continue;
 ...
 */
export const parseDatasheetDocument = (text: string): DatasheetDocument => {
    const sections: DatasheetSection[] = [];
    const rows: DatasheetRow[] = [];
    let current: DatasheetSection = { title: '', rows: [], notes: [] };
    const push = () => {
        if (current.rows.length || current.notes.length || current.title) sections.push(current);
    };

    for (const rawLine of String(text || '').split(/\r?\n/)) {
        const line = rawLine.replace(/\s+/g, ' ').trim();
        if (!line || RUNNING_HEAD.test(line)) continue;
...
/* ── 2) Aus den ZEILEN die Produktangaben ───────────────────────────────── */

/**
 * Die gesuchten Angaben, jede mit ihren Bezeichnungen (Englisch und Deutsch —
 * die Sprache des Blattes folgt dem Projekt) und der erwarteten Einheit.
 *
 * Die Einheit entscheidet mit: ein Blatt führt die Kühlleistung ZWEIMAL, in
 * kcal/h und in kW. Ohne die erwartete Einheit stünde die falsche Zahl auf der
 * Offerte.
 */
interface FieldSpec {
    labels: string[];
...
const findRow = (rows: DatasheetRow[], field: FieldSpec): DatasheetRow | null => {
    const wanted = field.labels.map(normalise);
    const matches = rows.filter((row) => wanted.includes(normalise(row.label)));
    if (!matches.length) return null;
    if (!field.unit) return matches[0] as DatasheetRow;
    return (matches.find((row) => row.unit === field.unit) ?? null);
};

const valueOf = (rows: DatasheetRow[], name: keyof typeof FIELDS): string | undefined => {
 ...
 */
const withUnitOnce = (value: string | undefined, unit: string): string | undefined => {
 ...
 */
export const specsFromDocument = (doc: DatasheetDocument): OspDatasheetSpecs => {
    ...
    if (heating) {
        specs.power = withUnitOnce(heating, 'kW') as string;
        specs.powerIsCooling = false;
        if (cooling) specs.coolingPower = withUnitOnce(cooling, 'kW') as string;
    } else if (cooling) {
        specs.power = withUnitOnce(cooling, 'kW') as string;
    ...
    if (glycol) specs.medium = /^(none|keine|kein|-)$/i.test(glycol) ? 'Wasser' : glycol;
    ...
    if (sound1m) specs.sound1m = withUnitOnce(sound1m, 'dB(A)') as string;
    if (sound10m) specs.sound10m = withUnitOnce(sound10m, 'dB(A)') as string;
    ...
    if (length && width && height) {
        specs.dimensions = withUnitOnce(`${length} x ${width} x ${height}`, 'mm') as string;
    } else {
        // Aus der Markdown-Fassung kommt die Abmessung als EIN Wert zurück.
        const whole = valueOf(rows, 'dimensions');
        if (whole) specs.dimensions = whole;
    }

    const weight = valueOf(rows, 'weight');
    if (weight) specs.weight = withUnitOnce(weight, 'kg') as string;
...
const mdEscape = (value: string): string => value.replace(/\|/g, '\\|');

/** Die Produktangaben als Tabelle — die Kurzfassung ganz oben. */
const summaryTable = (specs: OspDatasheetSpecs): string[] => {
    ...
    const present = rows.filter(([, value]) => Boolean(value)) as Array<[string, string]>;
    if (!present.length) return [];
    const out = ['## Produktangaben', '', '| Angabe | Wert |', '| --- | --- |'];
    for (const [label, value] of present) out.push(`| ${mdEscape(label)} | ${mdEscape(value)} |`);
    if (specs.technology) {
 ...
 */
export const buildDatasheetMarkdown = (
    ...
    const lines: string[] = [`# ${title}`, '', `_${origin}_`, '', ...summaryTable(specs)];

    for (const section of doc.sections) {
        if (!section.rows.length && !section.notes.length) continue;
 ...
 */
export const parseMarkdownDocument = (markdown: string): DatasheetDocument => {
    const sections: DatasheetSection[] = [];
    const rows: DatasheetRow[] = [];
    let current: DatasheetSection = { title: '', rows: [], notes: [] };
    const push = () => {
    ...
    for (const rawLine of String(markdown || '').split(/\r?\n/)) {
        ...
        if (line.startsWith('|')) {
            ...
            if (cells.length >= 2 && !/^-{2,}$/.test(cells[0] || '')) {
                ...
                if (label && value) {
                    const row: DatasheetRow = { label, unit: unit || null, value, raw: line };
                    current.rows.push(row);
                    rows.push(row);
                }
            }
