...
export const SIGNATURE_IMAGE_CID = "offitec-mail-signature";
...
const SIGNATURE_ALLOWED_TAGS =
    /^(b|strong|i|em|u|s|strike|ul|ol|li|br|p|div|span|font|h1|h2|h3|a|img|table|tbody|thead|tfoot|tr|td|th|hr|small|sup|sub|blockquote|pre|code)$/i;
...
const STYLE_RULE_RE =
    /^(color|background-color|background|font-size|font-family|font-weight|font-style|font|text-decoration|text-align|text-transform|line-height|letter-spacing|margin(?:-[a-z]+)?|padding(?:-[a-z]+)?|border(?:-[a-z]+)*|border-radius|width|height|max-width|min-width|vertical-align|display|float|opacity|white-space)\s*:\s*[^;]+$/i;
...
const numericAttr = (attrs: string, name: string): string => {
...
const colorAttr = (attrs: string, name: string): string => {
...
const alignAttr = (attrs: string, name: string): string => {
    const value = getAttr(attrs, name)?.trim();
...
export const sanitizeSignatureHtml = (html: string): string =>
    html
        ...
        .replace(/<\s*(\/?)\s*([a-z][a-z0-9]*)((?:[^>"']|"[^"]*"|'[^']*')*)>/gi, (_m, closing, tag, attrs) => {
            if (!SIGNATURE_ALLOWED_TAGS.test(tag)) return "";
            const lower = String(tag).toLowerCase();
            if (closing) return lower === "img" || lower === "br" || lower === "hr" ? "" : `</${lower}>`;
            ...
            if (lower === "img") {
                ...
                if (!src || !(DATA_IMAGE_RE.test(src) || HTTP_URL_RE.test(src))) return "";
                safeAttrs += ` src="${src}"`;
                ...
                if (alt && !/["'<>]/.test(alt)) safeAttrs += ` alt="${alt}"`;
            } else if (lower === "a") {
                ...
                if (face && /^[a-z0-9,'\s-]+$/i.test(face)) safeAttrs += ` face="${face}"`;
            }
            ...
            if (lower === "img") return `<img${safeAttrs} />`;
            return `<${lower}${safeAttrs}>`;
        });

export const signatureHtmlToText = (html: string): string =>
    html
        .replace(/<br\s*\/?\s*>/gi, "\n")
        .replace(/<\/(t[dh])\s*>/gi, " ")
...
export const signatureHasContent = (html: string | null | undefined): boolean => {
 ...
 * null döner.
 */
export const parseSignatureImage = (
    dataUrl: string | null | undefined,
): { contentType: string; contentBase64: string } | null => {
    if (!dataUrl) return null;
    const match = /^data:(image\/(?:png|jpeg));base64,([A-Za-z0-9+/=\s]+)$/.exec(dataUrl.trim());
    const contentType = match?.[1];
    const contentBase64 = match?.[2]?.replace(/\s+/g, "");
    if (!contentType || !contentBase64) return null;
    const bytes = Math.floor(contentBase64.length * 3 / 4);
    if (bytes > SIGNATURE_IMAGE_MAX_BYTES) return null;
    return { contentType, contentBase64 };
};

export interface SignatureParts {
    /** Mail gövdesinin sonuna eklenecek HTML bloğu ('' = imza yok). */
    html: string;
    /** text/plain gövdeye eklenecek karşılık ('' = imza yok). */
    text: string;
    /** İmzadaki tüm görsellerin CID'li inline ek listesi. */
 ...
 */
export const buildSignatureParts = (
    settings: { signatureHtml?: string | null; signatureImage?: string | null } | null | undefined,
): SignatureParts => {
    const empty: SignatureParts = { html: "", text: "", inlineImages: [] };
    if (!settings) return empty;

    const inlineImages: SignatureParts["inlineImages"] = [];

    let bodyHtml = "";
    if (signatureHasContent(settings.signatureHtml)) {
        bodyHtml = sanitizeSignatureHtml(String(settings.signatureHtml)).replace(
            /src="data:(image\/(?:png|jpeg|gif));base64,([A-Za-z0-9+/=]+)"/gi,
            (_m, contentType, contentBase64) => {
    ...
    const legacyImage = parseSignatureImage(settings.signatureImage);
    const legacyHtml = legacyImage
        ? `<div style="margin-top:8px"><img src="cid:${SIGNATURE_IMAGE_CID}" alt="" style="max-width:420px;height:auto" /></div>`
        : "";
    if (legacyImage) inlineImages.push({ cid: SIGNATURE_IMAGE_CID, ...legacyImage });

    if (!bodyHtml && !legacyHtml) return empty;

    const text = bodyHtml ? signatureHtmlToText(bodyHtml) : "";
