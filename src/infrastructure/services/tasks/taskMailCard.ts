import { BRAND_LOGO_CID } from "../mailBrand";
...
const TINTS: Record<string, { bg: string; fg: string }> = {
    [TASK_MAIL_BLUE]: { bg: "#e9f2ff", fg: "#0064d6" },
    [TASK_MAIL_RED]: { bg: "#ffecea", fg: "#d70015" },
};
...
export interface TaskMailCardInput {
    /** Der Absendername unter der Karte. */
    brand: string;
    /** Kurzes Stichwort in der Kapsel («Yeni görev», «Soru»). */
    kicker: string;
    accent: string;
    /** Rote Kapsel neben dem Stichwort, z. B. «Önemli». */
    badge?: string | null;
    heading: string;
    /** Block mit Kalenderblatt, z. B. die Fälligkeit. */
    date?: { at: Date; label: string; text: string } | null;
    greeting: string;
    lead: string;
    /** Der ruhige graue Kasten: Beschreibung bzw. die Frage selbst. */
    quote?: string | null;
...
const capsule = (text: string, bg: string, fg: string) =>
    `<td style="padding:0 3px;"><table role="presentation" cellpadding="0" cellspacing="0" border="0"><tr><td bgcolor="${bg}" style="${FONT}background:${bg};border-radius:999px;padding:5px 12px;font-size:12px;line-height:16px;font-weight:600;letter-spacing:.01em;color:${fg};white-space:nowrap;">${escapeHtml(text)}</td></tr></table></td>`;

/** Das Kalenderblatt: roter Kopf mit dem Monat, darunter der Tag. */
const dateTile = (date: Date) => `
...
<tr><td bgcolor="${TASK_MAIL_RED}" align="center" style="${FONT}background:${TASK_MAIL_RED};border-radius:11px 11px 0 0;padding:3px 0 2px;font-size:9.5px;line-height:12px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#ffffff;">${escapeHtml(part(date, { month: "short" }))}</td></tr>
...
const dateBlock = (date: NonNullable<TaskMailCardInput["date"]>) => `
...
const listRow = (row: TaskMailRow, last: boolean) => `
        ...
        <tr><td align="center" style="padding:36px 32px 28px;border-bottom:1px solid ${HAIRLINE};">
            <table role="presentation" cellpadding="0" cellspacing="0" border="0" align="center" style="margin:0 auto;border-collapse:separate;"><tr>
                <td width="56" height="56" align="center" valign="middle" bgcolor="#ffffff" style="width:56px;height:56px;background:#ffffff;border:1px solid ${HAIRLINE};border-radius:15px;box-shadow:0 2px 6px rgba(0,0,0,.06);">
                    <img src="cid:${BRAND_LOGO_CID}" width="34" height="34" alt="" style="display:block;margin:0 auto;width:34px;height:34px;border:0;" />
                </td>
            </tr></table>
