...
const toCandidate = (entityType: 'QUOTE' | 'ORDER', row: Record<string, any>): Candidate => ({
...
const collectQuoteCandidates = async (): Promise<Candidate[]> => {
 ...
 */
const collectOrderCandidates = async (): Promise<Candidate[]> => {
    const rows = await prisma.$queryRaw<Array<Record<string, any>>>(Prisma.sql`
        SELECT s.tenantId, s.leadDays, s.intervalDays,
...
const linkFor = (hit: Hit): string =>
    hit.entityType === 'QUOTE' ? `/sales/quotes/${hit.entityId}` : `/sales/orders/${hit.entityId}`;

/** Deutscher Ersatztext — die Oberfläche baut aus `meta` den Satz in der Sprache der Person. */
const fallbackTitle = (hit: Hit, daysLeft: number): string => {
...
const dueHits = async (candidates: Candidate[]): Promise<Hit[]> => {
    const now = new Date();
    const floor = now.getTime() - CATCH_UP_DAYS * 24 * 60 * 60 * 1000;
    const hits: Hit[] = [];
    for (const candidate of candidates) {
        const dueAt = latestDueStep(candidate.reference, candidate.leadDays, candidate.intervalDays, now);
        if (!dueAt || dueAt.getTime() <= floor) continue;
        hits.push({ ...candidate, dueAt });
    }
    if (hits.length === 0) return [];

    // Was im Nachholfenster schon gezündet hat, fällt weg — eine Abfrage.
    const fired = await prisma.reminderDispatch.findMany({
        where: {
    ...
    const key = (row: { entityType: string; entityId: string; dueAt: Date }) =>
        `${row.entityType}|${row.entityId}|${row.dueAt.getTime()}`;
...
export const startReminderEngine = (): void => {
