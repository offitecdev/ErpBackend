...
import type { ISalesSourceReader } from '../../domain/repositories/IProductionRepository';
import type { ProductionItemKind, SalesSourceLine, SalesSourceOrder, SalesSourceSnapshot } from '../../domain/entities/Production';
import { PRODUCTION_ORDER_TYPES, round2 } from '../../domain/services/production';
...
const inChunks = async <T,>(ids: string[], run: (part: string[]) => Promise<T[]>): Promise<T[]> => {
    const out: T[] = [];
...
const plainText = (value: unknown, max = 1000): string | null => {
...
const metaOf = (raw: unknown): Record<string, any> | null => {
    if (!raw) return null;
    try {
...
const kindOf = (itemType: unknown): ProductionItemKind =>
    String(itemType || '').toUpperCase() === 'SERVICE' ? 'SERVICE' : 'DEVICE';

export class PrismaSalesSourceReader implements ISalesSourceReader {
    async read(sourceTenantIds: string[]): Promise<SalesSourceSnapshot> {
        if (!sourceTenantIds.length) return { orders: [], lines: [] };
        ...
        const orderRows = await prisma.$queryRaw<any[]>(Prisma.sql`
            ...
            WHERE so.tenantId IN (${Prisma.join(sourceTenantIds)})
              AND so.orderType IN (${Prisma.join([...PRODUCTION_ORDER_TYPES])})
        `);

        const orders: SalesSourceOrder[] = orderRows.map((row) => ({
            id: String(row.id),
        ...
        const orderByTender = new Map<string, string>();
        for (const row of orderRows) if (row.tenderId) orderByTender.set(String(row.tenderId), String(row.id));
        const tenderIds = [...orderByTender.keys()];

        const [positionRows, materialRows, expenseRows] = await Promise.all([
            inChunks(tenderIds, (part) => prisma.$queryRaw<any[]>(Prisma.sql`
                ...
                WHERE pos.tenderId IN (${Prisma.join(part)})
                  AND pos.rowType IN ('PRODUCT', 'CUSTOM')
            `)),
            inChunks(orderIds, (part) => prisma.$queryRaw<any[]>(Prisma.sql`
                ...
                WHERE em.salesOrderId IN (${Prisma.join(part)})
            `)),
            inChunks(orderIds, (part) => prisma.$queryRaw<any[]>(Prisma.sql`
                SELECT pe.id, pe.salesOrderId, pe.expenseType, pe.amount, pe.description, pe.documentLine
                FROM ProjectExpense pe
                WHERE pe.salesOrderId IN (${Prisma.join(part)})
            `)),
        ]);

        const lines: SalesSourceLine[] = [];
        ...
        for (const row of positionRows) {
            ...
            lines.push({
                ...
                unit: text(row.unit),
                unitPrice,
                totalPrice: round2(quantity * unitPrice * (1 - discount / 100)),
