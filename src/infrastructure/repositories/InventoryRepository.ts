...
let naturalSortKeySupported: boolean | null = null;
const supportsNaturalSortKey = async (): Promise<boolean> => {
    if (naturalSortKeySupported !== null) return naturalSortKeySupported;
    try {
        await (prisma as any).$queryRawUnsafe("SELECT NATURAL_SORT_KEY('a') AS k");
...
const naturalNameOrderSql = (alias: string, direction: 'ASC' | 'DESC', natural: boolean): string => {
...
export class InventoryRepository implements IInventoryRepository {
    ...
    async getArticleStockSummaryPaged(
        ...
        if (requestedSort === NATURAL_NAME_SORT) {
            const { clauses, values } = buildRawFilter();
            const whereSql = clauses.join(' AND ');
            const orderBySql = naturalNameOrderSql(
                'a',
                sortDirection === 'asc' ? 'ASC' : 'DESC',
                await supportsNaturalSortKey(),
            );
            const [countRows, articleRows] = await Promise.all([
                (prisma as any).$queryRawUnsafe(
                    `SELECT COUNT(*) AS total FROM \`Article\` a WHERE ${whereSql}`,
                ...
                (prisma as any).$queryRawUnsafe(
                    `SELECT a.\`id\`, a.\`articleCode\`, a.\`name\`, a.\`description\`,
            ...
            return {
                items: (articleRows as any[]).map(mapLeanRow),
                total: Math.max(0, Number((countRows as any[])?.[0]?.total) || 0),
                page,
                pageSize,
            };
