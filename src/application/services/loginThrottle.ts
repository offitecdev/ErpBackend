...
const keyOf = (email: string): string => String(email ?? '').trim().toLowerCase();

/** Wann ein Eintrag nichts mehr aussagt und weg darf. */
const isStale = (record: AttemptRecord, now: number): boolean =>
    record.lockedUntil <= now && now - record.lastFailureAt > FAILURE_WINDOW_MS;

const prune = (now: number): void => {
    for (const [key, record] of records) {
        if (isStale(record, now)) records.delete(key);
    }
    ...
    if (records.size > MAX_TRACKED) {
        ...
        for (const [key] of oldestFirst.slice(0, records.size - MAX_TRACKED)) records.delete(key);
    }
};

/** Wartezeit nach `failures` Fehlversuchen: 1, 2, 4, 8, 16, … Minuten. */
const lockDurationFor = (failures: number): number => {
 ...
 */
export const assertLoginAllowed = (email: string): void => {
    ...
    if (record.lockedUntil > now) {
        ...
        throw new TooManyAttemptsError(
            `Çok fazla hatalı giriş denemesi. Lütfen ${minutes} dakika sonra tekrar deneyin.`,
            retryAfterSeconds,
        );
    }
};
...
export const recordLoginFailure = (email: string): void => {
    ...
    if (records.size > MAX_TRACKED) prune(now);
};
...
export const resetLoginThrottle = (): void => records.clear();
export const loginThrottleSize = (): number => records.size;
