 ...
 *
 *   «Wird ein Auftrag gelöscht, fallen die daran hängenden Nachträge mit.
 *    Wird ein Lieferauftrag gelöscht, wird er sofort wieder ein Offert-Entwurf.»
 *
 * Diese Regeln galten bisher nur auf dem Weg über das Projekt
 * (`DELETE /projects/:id/sales-orders/:salesOrderId`) und liessen einen
 * Lieferauftrag — der kein Projekt hat — überhaupt nicht löschen. Sie stehen
 * darum hier, EINMAL, und werden von beiden Adressen benutzt.
 *
 * ── WAS SEIT DEM 06.09.2026 NICHT MEHR HIER GESCHIEHT ────────────────────────
 * Der letzte Auftrag nimmt das PROJEKT NICHT mehr mit. «Zurück in den Entwurf»
 * lässt es als leere Planung stehen, «Storno» stellt es auf CANCELLED, und
 * gelöscht wird ein Projekt nur noch über seinen eigenen Weg (`deleteProject`,
 * und auch der erst, wenn nichts mehr daran hängt). Dieses Modul ist deshalb
 * nur noch die MECHANIK des Auftragsschnitts; welche Handlung überhaupt
 * offensteht, entscheidet `documentLifecycle.ts`.
 *
 * Was dabei NICHT verschwindet: Rapporte, Spesen und Termine eines Nachtrags.
 * Der Nachtrag ist nur der Rechnungsschnitt über eine Zeitscheibe des
 * Hauptauftrags; fällt er, werden seine Sätze auf den Hauptauftrag
 * zurückgestempelt und warten dort auf den nächsten Schnitt. Zusatzmaterial
 * geht in beiden Fällen an den Lagerbestand zurück.
 */

type Tx = any;

export interface DeletableSalesOrder {
    id: string;
 ...
 */
export const revertTendersToDraft = async (
    tx: Tx,
    tenantId: string,
    employeeId: string,
...
export const salesOrderFamilyIds = async (db: Tx, order: DeletableSalesOrder, tenantId: string): Promise<string[]> => {
    if (order.parentSalesOrderId) return [order.id];
    const addons: any[] = await db.salesOrder.findMany({
        where: { parentSalesOrderId: order.id, tenantId },
        select: { id: true },
    });
    return [order.id, ...addons.map((addon: any) => addon.id)];
};
 ...
 */
export const assertSalesOrderDeletable = async (
    db: Tx,
    order: DeletableSalesOrder,
    tenantId: string,
...
const restockExtraMaterials = async (
    tx: Tx,
    where: any,
    context: { tenantId: string; employeeId: string; referenceId: string },
): Promise<void> => {
    const rows: any[] = await tx.projectExtraMaterial.findMany({
        where,
    ...
    if (rows.length) {
        await tx.projectExtraMaterial.deleteMany({ where: { id: { in: rows.map((row: any) => row.id) } } });
    }
};
 ...
 */
export const purgeProjectWithin = async (
    tx: Tx,
    opts: { projectId: string; tenantId: string; employeeId: string },
): Promise<void> => {
...
export interface SalesOrderDeletionResult {
    /**
     * IMMER `false` seit dem 06.09.2026 — das Projekt fällt nicht mehr mit
     * einem Auftrag. Das Feld bleibt, weil ältere Oberflächen es lesen und
     * daraufhin von der Projektseite wegnavigieren würden.
     */
    projectDeleted: boolean;
    /** Mitgelöschte Nachträge. */
    addonIds: string[];
}
 ...
 */
export const deleteSalesOrderWithin = async (
    tx: Tx,
    opts: {
        order: DeletableSalesOrder;
        tenantId: string;
...
): Promise<SalesOrderDeletionResult> => {
    ...
    if (isAddon && projectId) {
        ...
        const previousAddon = siblings
            .filter((sibling: any) => new Date(sibling.createdAt).getTime() < new Date(order.createdAt).getTime())
        ...
        await restockExtraMaterials(
            tx,
            { projectId, OR: [{ salesOrderId: order.id }, legacyWindow] },
            { tenantId, employeeId, referenceId: projectId },
        );
    ...
    if (!isAddon && projectId) {
        ...
        await restockExtraMaterials(
            tx,
            { projectId, salesOrderId: { in: familyIds } },
            { tenantId, employeeId, referenceId: projectId },
        );
    ...
    if (!isAddon) {
        await revertTendersToDraft(
            ...
            employeeId,
            [order.tenderId],
            `${order.orderNumber || 'Auftrag'} silindi; teklif taslaga dondu.`,
        );
    }
