...
export const HOLIDAY_COUNTRIES = ['TR'] as const;
...
export interface HolidayCatalogEntry {
    /** Stabiler Schlüssel — an ihm erkennt die Liste einen schon übernommenen Tag. */
    key: string;
    /** ISO-Kalendertag (YYYY-MM-DD) im abgefragten Jahr. */
    date: string;
    names: HolidayNames;
    /** true = religiöses Fest (Ramazan-/Kurban-Bayram). */
    religious: boolean;
    /** Arife: ein halber Tag; er zählt mit 0.5 statt 1. */
    halfDay: boolean;
}

/** Die festen Tage des türkischen Kalenders: Monat (1-12) und Tag. */
const FIXED_TR: ReadonlyArray<{ key: string; month: number; day: number; halfDay?: boolean; names: HolidayNames }> = [
    {
 ...
 */
const RAMAZAN_FIRST_DAY: Record<number, string> = {
    2024: '2024-04-10',
    2025: '2025-03-30',
    2026: '2026-03-20',
    2027: '2027-03-09',
...
const KURBAN_FIRST_DAY: Record<number, string> = {
    2024: '2024-06-16',
    2025: '2025-06-06',
    2026: '2026-05-27',
    2027: '2027-05-16',
...
const feast = (
    ...
    length: number,
    names: HolidayNames,
    eveNames: HolidayNames,
): HolidayCatalogEntry[] => {
    const rows: HolidayCatalogEntry[] = [{
        key: `${prefix}.eve`,
 ...
 */
export const holidayCatalog = (year: number, country: string = 'TR'): HolidayCatalogEntry[] => {
    if (country !== 'TR') return [];
    if (!Number.isInteger(year) || year < 1990 || year > 2100) return [];

    const rows: HolidayCatalogEntry[] = FIXED_TR.map((entry) => ({
        key: entry.key,
    ...
    if (ramazan) {
        rows.push(...feast(
            'tr.ramazan', ramazan, 3,
            { tr: 'Ramazan Bayramı', de: 'Ramazan-Fest', en: 'Ramadan Feast' },
            { tr: 'Ramazan Bayramı Arifesi', de: 'Ramazan-Fest, Vorabend', en: 'Ramadan Feast Eve' },
        ));
    }
    ...
    if (kurban) {
        rows.push(...feast(
            'tr.kurban', kurban, 4,
            { tr: 'Kurban Bayramı', de: 'Opferfest', en: 'Feast of Sacrifice' },
            { tr: 'Kurban Bayramı Arifesi', de: 'Opferfest, Vorabend', en: 'Feast of Sacrifice Eve' },
        ));
    }
...
export const holidayCatalogYears = (): number[] =>
    Object.keys(RAMAZAN_FIRST_DAY).map(Number).sort((a, b) => a - b);
